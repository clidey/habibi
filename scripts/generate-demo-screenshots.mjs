...
const service = spawn('node', [join(root, 'dist/server.js')], {
  cwd:root,
  env:{ ...process.env, HABIBI_ROOT:root, HABIBI_DATA_ROOT:join(tmpdir(), 'habibi-demo-data'), HABIBI_PORT:String(port) },
  stdio:'ignore',
});
...
try {
  for (let attempt = 0; attempt < 30; attempt += 1) {
    try { if ((await fetch(url)).ok) break; } catch (_) { /* wait for local service */ }
    await sleep(100);
  }
  const shots = [
  ...
  for (const [name, demo] of shots) {
    ...
    execFileSync(renderer, [`${url}/?demo=${demo}`, uncropped, '1024', '720'], { stdio:'inherit' });
    ...
    execFileSync('sips', ['--cropToHeightWidth', '720', '988', '--cropOffset', '0', '0', uncropped, '--out', file], { stdio:'ignore' });
    ...
    execFileSync('sips', ['-s', 'format', 'jpeg', '-s', 'formatOptions', '100', file, '--out', flattenedJpeg], { stdio:'ignore' });
    execFileSync('sips', ['-s', 'format', 'png', flattenedJpeg, '--out', flattenedPng], { stdio:'ignore' });
