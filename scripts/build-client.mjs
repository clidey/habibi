...
const lucideIcons = [
  ...
  ['Moon', 'moon'], ['OctagonX', 'octagon-x'], ['PanelTop', 'panel-top'],
  ['PanelTopClose', 'panel-top-close'], ['Paperclip', 'paperclip'], ['Pause', 'pause'],
...
const lucideImports = lucideIcons.map(([binding, file]) => `import ${binding} from 'lucide/dist/esm/icons/${file}.js';`).join('\n');
const lucideMap = lucideIcons.map(([binding]) => `${binding === 'FileIcon' ? 'File' : binding}:${binding}`).join(',');
await esbuild.build({
  stdin:{
    contents:`
