...
function nonEmptyStrings(value: unknown, field: string, manifestId: string): string[] {
  if (!Array.isArray(value) || value.some((item) => typeof item !== 'string' || !item.trim())) {
    throw new Error(
      `Invalid skill manifest ${manifestId}: ${field} must be a non-empty string array`,
    );
  }
...
function pluginTools(
  value: unknown,
  manifestId: string,
): ReadonlyArray<Readonly<{ name: string; description: string; readOnly: boolean }>> | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || !value.length)
    throw new Error(`Invalid skill manifest ${manifestId}: tools must be a non-empty array`);
  const seen = new Set<string>();
  return Object.freeze(
    value.map((item) => {
      if (!item || typeof item !== 'object' || Array.isArray(item))
        throw new Error(`Invalid skill manifest ${manifestId}: tool must be an object`);
      ...
      if (
        ...
        throw new Error(`Invalid skill manifest ${manifestId}: invalid tool declaration`);
      seen.add(name);
      return Object.freeze({ name, description, readOnly: candidate.readOnly });
    }),
  );
}
...
export function parseSkillManifest(value: unknown): Readonly<SkillManifest> {
  ...
  if (!identifier.test(id))
    throw new Error(`Invalid skill manifest ${id || 'unknown'}: id must be kebab-case`);
  if (!name) throw new Error(`Invalid skill manifest ${id}: name is required`);
  if (!version.test(manifestVersion))
    throw new Error(`Invalid skill manifest ${id}: version must be semantic versioning`);
  if (typeof kind !== 'string' || !kinds.has(kind as SkillKind))
    throw new Error(`Invalid skill manifest ${id}: unsupported kind`);
