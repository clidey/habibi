...
function createMcpBridge({ root, fs }) {
  ...
  const read = () => {
    try { return JSON.parse(fs.readFileSync(configFile, 'utf8')).servers || []; } catch (_) { return []; }
  };
  ...
  const discover = async serverId => {
    ...
    try {
      const [{ Client }, transports] = await Promise.all([
        ...
        definition.transport === 'stdio' ? import('@modelcontextprotocol/sdk/client/stdio.js') : import('@modelcontextprotocol/sdk/client/streamableHttp.js')
      ]);
      const transport = definition.transport === 'stdio'
        ? new transports.StdioClientTransport({ command:definition.command, args:definition.args || [], env:definition.env })
      ...
      return { ok:true, tools:(response.tools || []).map(tool => ({ id:asHabibiToolName(definition.id, tool.name), serverId:definition.id, name:tool.name, description:tool.description || '', inputSchema:tool.inputSchema, annotations:tool.annotations || {} })) };
    } catch (error) { return { ok:false, error:error.message || 'Could not connect to the MCP server.', tools:[] }; }
