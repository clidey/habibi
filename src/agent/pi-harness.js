...
async function loadRuntime() {
  ...
  ]).then(([agent, ai, openai, anthropic, google]) => ({
    ...agent,
    ...ai,
    ...openai,
    ...anthropic,
...
const providerEndpoint = (provider, endpoint) =>
  provider === 'ollama' ? `${endpoint.replace(/\/$/, '')}/v1` : endpoint.replace(/\/$/, '');
const modelInput = (model) =>
  /(?:vision|\bvl\b|llava|qwen.*vl|gpt-4o|gpt-4\.1|gpt-5|chat-latest|gemini|claude)/i.test(model)
    ? ['text', 'image']
    : ['text'];
  ...
  async function complete({ config, systemPrompt, messages, tools = [] }) {
    ...
    const streams =
      api === 'anthropic-messages'
        ? runtime.anthropicMessagesApi()
        : api === 'google-generative-ai'
          ? runtime.googleGenerativeAIApi()
          : runtime.openAICompletionsApi();
    ...
    models.setProvider(
      runtime.createProvider({
        ...
        baseUrl: endpoint,
        auth: {
          apiKey: { name: provider, resolve: async () => ({ auth: apiKey ? { apiKey } : {} }) },
        },
        models: [model],
        api: streams,
      }),
    ...
    finalAssistant ||= [...agent.state.messages]
      .reverse()
      .find((message) => message.role === 'assistant');
    return {
      text: textFromMessage(finalAssistant),
      error: agent.state.errorMessage,
      toolEvents: [...agent.state.messages].filter((message) => message.role === 'toolResult'),
    };
  }
