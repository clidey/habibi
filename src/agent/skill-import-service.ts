...
function parseInstruction(
  ...
  source: ImportedSkillSource,
  kind: ImportedSkillKind,
): InternalImportedSkill | null {
  const raw = safeRead(file);
  if (!raw) return null;
  const frontmatter = raw.match(/^---\s*\n([\s\S]*?)\n---\s*\n?/);
  const metadata = frontmatter?.[1] || '';
  const title =
    metadata.match(/^name:\s*["']?(.+?)["']?\s*$/m)?.[1] ||
    raw.match(/^#\s+(.+)$/m)?.[1] ||
    path.basename(file, path.extname(file));
  const description =
    metadata.match(/^description:\s*["']?(.+?)["']?\s*$/m)?.[1] ||
    raw
      .split('\n')
      .map((line) => line.trim())
...
function mcpEntries(file: string, sourceLabel: string): InternalImportedSkill[] {
  ...
  return Object.entries(servers as Record<string, unknown>).flatMap(([name, definition]) => {
    ...
    return [
      {
        id: idFor('mcp', 'mcp-server', `${file}:${name}`),
        source: 'mcp' as const,
        kind: 'mcp-server' as const,
        name,
        description: `${sourceLabel} MCP server · ${transport === 'http' ? 'HTTP' : 'local stdio'}`,
        location: file,
        executable: true,
        transport,
        mcp: value,
...
export function createSkillImportService({
  ...
  const discover = (): InternalImportedSkill[] => {
    const codexSkills = [
      ...
      ...roots.map((item) => path.join(item, '.codex', 'skills')),
    ]
      .flatMap((folder) => walk(folder, (file) => path.basename(file).toLowerCase() === 'skill.md'))
      .map((file) => parseInstruction(file, 'codex', 'instruction'))
      .filter((skill): skill is InternalImportedSkill => Boolean(skill));
    const claudeSkills = [
      path.join(home, '.claude', 'skills'),
      ...roots.map((item) => path.join(item, '.claude', 'skills')),
    ]
      .flatMap((folder) => walk(folder, (file) => path.basename(file).toLowerCase() === 'skill.md'))
      .map((file) => parseInstruction(file, 'claude', 'instruction'))
      .filter((skill): skill is InternalImportedSkill => Boolean(skill));
    const claudeCommands = [
      path.join(home, '.claude', 'commands'),
      ...roots.map((item) => path.join(item, '.claude', 'commands')),
    ]
      .flatMap((folder) => walk(folder, (file) => file.toLowerCase().endsWith('.md')))
      .map((file) => parseInstruction(file, 'claude', 'command'))
      .filter((skill): skill is InternalImportedSkill => Boolean(skill));
    const mcp = [
      ...roots
        .map((item) => path.join(item, '.mcp.json'))
        .flatMap((file) => mcpEntries(file, 'Project')),
      ...roots
        .map((item) => path.join(item, '.habibi', 'mcp-servers.json'))
        .flatMap((file) => mcpEntries(file, 'Habibi')),
  ...
  const find = (id: string): InternalImportedSkill | undefined =>
    discover().find((skill) => skill.id === id);
  const audit = (event: AuditEvent): void => {
    try {
      ...
      /* Audit is best effort; an unavailable disk must not widen permissions. */
    }
  };

  const withMcpClient = async <T>(
    skill: InternalImportedSkill,
    run: (client: any) => Promise<T>,
  ): Promise<T> => {
    const definition = skill.mcp || {};
    const { Client } = await import('@modelcontextprotocol/sdk/client/index.js');
    const transport =
      skill.transport === 'http'
        ? new (
            await import('@modelcontextprotocol/sdk/client/streamableHttp.js')
          ).StreamableHTTPClientTransport(new URL(String(definition.url)))
        : new (await import('@modelcontextprotocol/sdk/client/stdio.js')).StdioClientTransport({
            ...
            env:
              definition.env && typeof definition.env === 'object'
                ? (definition.env as Record<string, string>)
                : undefined,
          });
    if (skill.transport === 'stdio' && !String(definition.command || '').trim())
      throw new Error('This MCP server has no local command.');
  ...
  const preview = async (id: string): Promise<Preview | { ok: false; error: string }> => {
    ...
    if (skill.kind !== 'mcp-server')
      return {
        ...
        action: `Open ${skill.source === 'codex' ? 'Codex' : 'Claude Code'} with this local instruction`,
        prompt: skill.prompt?.slice(0, 1200) || '',
      };
    try {
      const response = await withMcpClient<{
        tools?: Array<{
          name: unknown;
          description?: unknown;
          inputSchema?: unknown;
      ...
      return {
        ok: true,
        skill: publicSkill(skill),
        action: 'Choose an MCP tool and review its JSON input',
        tools,
      };
    } catch (error) {
      return {
        ok: false,
        error: error instanceof Error ? error.message : 'Could not inspect this MCP server.',
      };
    }
  };
  ...
  const execute = async ({
    ...
    try {
      if (skill.kind === 'mcp-server') {
        if (!toolName || !toolInput || typeof toolInput !== 'object' || Array.isArray(toolInput))
          return { ok: false, error: 'Provide an MCP tool and a JSON object input.' };
        const result = await withMcpClient(skill, (client) =>
      ...
      const promptFile = path.join(
        fs.mkdtempSync(path.join(os.tmpdir(), 'habibi-skill-')),
        'prompt.txt',
      );
      fs.writeFileSync(promptFile, prompt, { encoding: 'utf8', mode: 0o600 });
      const process = spawn('osascript', ['-e', terminalScript, root, promptFile, agent], {
        detached: true,
        stdio: 'ignore',
      });
      process.unref();
      ...
      audit({
        ...
        source: skill.source,
        kind: skill.kind,
        action: skill.kind === 'mcp-server' ? 'call_mcp' : 'launch_agent',
        outcome: 'failed',
      });
      return {
        ok: false,
        error: error instanceof Error ? error.message : 'Could not run this imported skill.',
      };
    }
  };
