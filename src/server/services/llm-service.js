...
function inferIntent(text = '') {
  ...
  if (
    /\bwhatsapp\b|\b(?:message|text|ping)\s+(?:a\s+)?(?:person|contact|someone|them)\b/i.test(value)
  )
    return {
      kind: 'whatsapp_draft',
      label: 'WhatsApp draft',
      detail: 'Find the recipient, review the recent chat, then show a draft for approval.',
    };
  if (/\b(calendar|schedule|meeting|event|book|remind)\b/.test(value))
    return {
      kind: 'calendar_draft',
      label: 'Calendar draft',
      detail: 'Extract the time and attendees, then show an event draft for approval.',
    };
  if (/\b(email|gmail|mail)\b/.test(value))
    return {
      kind: 'email_draft',
      label: 'Email draft',
      detail: 'Prepare a reply or new email for approval.',
    };
  return null;
}
...
function createLlmService({ root, fs, spawn }) {
  ...
  const command = (program, args, input) =>
    new Promise((resolve) => {
      ...
      child.on('close', (code) =>
        resolve({ ok: code === 0, stdout: stdout.trim(), error: stderr.trim() }),
      );
  ...
  const saveKey = async (provider, apiKey) =>
    command(
      'security',
      ['add-generic-password', '-U', '-s', keychainService, '-a', keyAccount(provider), '-w'],
      `${apiKey}\n${apiKey}\n`,
    );
  ...
  const validateOpenAi = async ({ apiKey, model }) => {
    try {
      const response = await fetch(
        `${PROVIDERS.openai.endpoint}/models/${encodeURIComponent(model)}`,
        {
      ...
      if (response.status === 401 || response.status === 403)
        return 'OpenAI rejected that API key. Create an API key in the OpenAI platform and try again.';
      if (response.status === 429)
        return 'OpenAI accepted the key, but the API account has no available quota or is rate-limited. Check API billing and usage limits, then try again.';
      if (response.status === 404)
        return `This OpenAI API key cannot use the “${model}” model. Choose another model and try again.`;
  ...
  const configured = async () => {
    ...
    if (!config.provider || !PROVIDERS[config.provider])
      return { ok: true, configured: false, providers: PROVIDERS };
    ...
    return {
      ...
      endpoint: resolveEndpoint(config.provider, config.endpoint),
      providers: PROVIDERS,
    };
  };
  const configure = async ({ provider, model, endpoint, apiKey }) => {
    ...
    const definition = PROVIDERS[provider];
    const selectedModel =
      typeof model === 'string' && model.trim() ? model.trim() : definition.model;
    const suppliedKey = typeof apiKey === 'string' ? apiKey.trim() : '';
    if (definition.kind === 'external' && !suppliedKey && !(await getKey(provider)))
    ...
    writeConfig({
      provider,
      model: selectedModel,
      endpoint: resolveEndpoint(provider, endpoint),
  ...
  const models = async ({ provider, endpoint, apiKey } = {}) => {
    ...
    try {
      ...
      const key =
        typeof apiKey === 'string' && apiKey.trim() ? apiKey.trim() : await getKey(provider);
  ...
  const complete = async ({ messages, systemPrompt: overrideSystemPrompt } = {}) => {
    ...
    if (!state.configured)
      return { ok: false, needsConfiguration: true, error: 'Choose a model provider first.' };
    const config = readConfig();
    ...
    const supportsVision = /(?:vision|\bvl\b|llava|qwen.*vl|gpt-4o|gpt-4\.1|gemini|claude)/i.test(
      model,
    );
    const attachmentNote = (attachments) => {
      ...
      const pastedText = attachments
        .filter((item) => item.mime === 'text/plain' && typeof item.text === 'string')
        .map((item) => `\n\n[Attached text: ${item.name}]\n${item.text.slice(0, 24_000)}`)
        .join('')
        .slice(0, 48_000);
      return summary + pastedText;
    };
    const normalizedMessages = messages.slice(-20).map(({ role, text, content, attachments }) => ({
      role: role === 'assistant' ? 'assistant' : 'user',
      content: (text || content || '') + attachmentNote(attachments),
      attachments: attachments || [],
    }));
    const payloadMessages = [
      { role: 'system', content: system },
      ...normalizedMessages.map(({ role, content }) => ({ role, content })),
    ];
    const lastUser = [...normalizedMessages].reverse().find((message) => message.role === 'user');
    const imageAttachments = supportsVision
      ? (lastUser?.attachments || []).filter(
          (item) => /^image\//.test(item.mime || '') && /^data:image\//.test(item.dataUrl || ''),
        )
      : [];
    try {
      let response;
      ...
      if (!imageAttachments.length) {
        try {
          const result = await getPiHarness().complete({
      ...
      if (provider === 'ollama') {
        ...
        if (imageAttachments.length)
          ollamaMessages[ollamaMessages.length - 1].images = imageAttachments.map(
            (item) => item.dataUrl.split(',')[1],
          );
      ...
      if (provider === 'anthropic') {
        ...
        if (imageAttachments.length)
          anthropicMessages[anthropicMessages.length - 1].content = [
            ...
            ...imageAttachments.map((item) => ({
              type: 'image',
              source: { type: 'base64', media_type: item.mime, data: item.dataUrl.split(',')[1] },
            })),
          ];
        response = await fetch(`${endpoint}/v1/messages`, {
          ...
          body: JSON.stringify({ model, max_tokens: 900, system, messages: anthropicMessages }),
        });
        const data = await response.json();
      ...
      if (provider === 'gemini') {
        const contents = payloadMessages.slice(1).map((message) => ({
        ...
        if (imageAttachments.length)
          contents[contents.length - 1].parts.push(
            ...imageAttachments.map((item) => ({
              inlineData: { mimeType: item.mime, data: item.dataUrl.split(',')[1] },
            })),
          );
      ...
      response = await fetch(`${endpoint}/chat/completions`, {
        ...
        headers: {
          'Content-Type': 'application/json',
          ...(provider === 'lmstudio' ? {} : { Authorization: `Bearer ${apiKey}` }),
        },
        body: JSON.stringify({ model, messages: openAiMessages, temperature: 0.4 }),
      });
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error?.message || `${definition.label} could not answer`);
      return {
        ok: true,
      ...
      return { ok: false, error: error.message || 'The provider is unavailable.', provider, model };
    }
  };
  const route = async ({ text, context = '' }) => {
    const request = String(text || '').trim();
    const providerMatch = request.match(
      /^\s*(?:ask|use|open)\s+(chatgpt|claude|gemini)\s+(?:(?:to|about)\s+)?(.+)$/i,
    );
    if (providerMatch) {
        ...
        try {
          query =
            String(
              ...
              .trim()
              .slice(0, 800) || original;
        } catch (_) {
          /* The original request is a safe fallback. */
        }
      }
    ...
    const directQuestion =
      /^(?:tell(?:\s+me)?|explain|define|what(?:'s|\s+is|\s+are)?|who(?:\s+is|\s+are)?|why|how|compare|difference\s+between|help\s+me\s+understand)\b/i.test(
        request,
      );
    const needsLiveLookup =
      /\b(?:latest|today|right now|current(?:ly)?|news|weather|price|prices|availability|available|near me|nearby|schedule|score|stock)\b/i.test(
        request,
      );
    ...
    const result = await complete({
      messages: [{ role: 'user', text: `Context: ${context}\nRequest: ${request}` }],
      systemPrompt,
    });
    ...
    try {
      ...
      const query = String(parsed.query || request)
        .trim()
        .slice(0, 300);
      ...
      return {
        ...
        provider,
        query,
        checkin: /^\d{4}-\d{2}-\d{2}$/.test(parsed.checkin || '') ? parsed.checkin : null,
        checkout: /^\d{4}-\d{2}-\d{2}$/.test(parsed.checkout || '') ? parsed.checkout : null,
        adults: Number.isInteger(parsed.adults) && parsed.adults > 0 ? parsed.adults : null,
      };
    } catch (_) {
  ...
  const mailSearchPlan = async (query) => {
    ...
    if (!state.configured || !['ollama', 'lmstudio'].includes(state.provider) || !text)
      return fallback;
    ...
    try {
      ...
      const validDate = (date) =>
        /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? String(date) : '';
      return {
        ...
        subject: String(value.subject || '')
          .trim()
          .slice(0, 200),
        after: validDate(value.after),
        before: validDate(value.before),
  ...
  const planFileInvestigation = async ({ history = [] }) => {
    ...
    const contextualFallback = () => {
      ...
      const query = [...turns]
        .reverse()
        .map((text) => text.toLowerCase().match(/[a-z0-9][a-z0-9_-]{1,}/g) || [])
        .map((tokens) =>
          tokens
        ...
        .find(Boolean);
      const asksForFiles =
        /\b(?:find|search|show|give|locate|open|latest|recent|yesterday|last night|files?|folders?)\b/i.test(
          turns.join(' '),
        );
    ...
    if (!state.configured || !['ollama', 'lmstudio'].includes(state.provider))
      return contextualFallback() || { phase: 'not_applicable' };
    ...
    try {
      ...
      if (plan.phase === 'search' && Array.isArray(plan.queries)) {
        const planned = plan.queries
          ...
          .map((query) =>
            query
              .replace(/\b(?:19|20)\d{2}\b/g, (year) => (latestUserText.includes(year) ? year : ''))
              .replace(/\s+/g, ' ')
              .trim(),
          )
        ...
        const literalProbes = (latestUserText.match(/\b[A-Z][A-Z0-9_-]{1,}\b/g) || []).map(
          (value) => value.trim(),
        );
        const queries = [...literalProbes, ...planned]
          .filter(
            (query, index, all) =>
              all.findIndex((value) => value.toLowerCase() === query.toLowerCase()) === index,
          )
          .slice(0, 3);
      ...
      return question
        ...
        : contextualFallback() || { phase: 'not_applicable' };
    } catch (_) {
      return contextualFallback() || { phase: 'not_applicable' };
    }
  };
  const rankFileCandidates = async ({ history = [], candidates = [] }) => {
    ...
    const state = await configured();
    if (!state.configured || !['ollama', 'lmstudio'].includes(state.provider))
      return { ids: candidates.map((file) => file.path) };
    ...
    const result = await complete({
      messages: [
        ...history,
        { role: 'user', text: `Candidates to review locally:\n${JSON.stringify(candidateText)}` },
      ],
      systemPrompt,
    });
    ...
    try {
      ...
      return {
        ids: Array.isArray(ranked.ids)
          ? ranked.ids
              .map(String)
              .filter((id) => allowed.has(id))
              .slice(0, 8)
          : [],
        summary: String(ranked.summary || '')
          .trim()
          .slice(0, 300),
      };
    } catch (_) {
