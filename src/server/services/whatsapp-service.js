...
function createWhatsAppService({ root, fs, spawn, openwaClient }) {
  ...
  const saveSnapshot = (chats) => {
    ...
    try {
      ...
      /* A live session remains usable when disk is unavailable. */
    }
  };
  ...
  const dedupeChats = (chats) => {
    ...
    for (const chat of chats || []) {
      ...
      const newest =
        Number(chat.timestamp || 0) >= Number(previous.timestamp || 0) ? chat : previous;
  ...
  const normalizeHistory = (messages) =>
    (Array.isArray(messages) ? messages : messages?.messages || messages?.items || []).map(
      (message) => ({
        ...message,
  ...
  const warmPictures = async (session, ids) => {
    ...
    try {
      const result = await request(
        `/api/sessions/${encodeURIComponent(session.id)}/contacts/profile-pictures?ids=${encodeURIComponent(missing.join(','))}`,
      );
      const pictures = result?.pictures || result || {};
      missing.forEach((id) => {
        if (pictures[id]) pictureCache.set(id, pictures[id]);
      });
    } catch (_) {
      /* Avatars are optional enrichment. */
    }
  };
  ...
  const withReady = async (response, json, action) => {
    try {
      ...
      if (!session || session.status !== 'ready')
        return json(response, { ok: false, error: 'WhatsApp is not connected' });
      return action(session);
    } catch (_) {
      return json(response, { ok: false, error: 'WhatsApp is unavailable' });
    }
  };
  ...
  const readBody = (requestToRead) =>
    new Promise((resolve, reject) => {
      let body = '';
      requestToRead.on('data', (chunk) => {
        body += chunk;
        if (body.length > maxBodyBytes) {
          requestToRead.destroy();
  ...
  async function handle({
    ...
    if (url.pathname === '/api/openwa/status' && httpRequest.method === 'GET') {
      return sessionState()
        .then((state) => json(response, { ok: true, ...state }))
        .catch(() => json(response, { ok: false }));
    }
    if (url.pathname === '/api/openwa/connect' && httpRequest.method === 'POST') {
      return openwaClient
        ...
        .then(sessionState)
        .then((state) => json(response, { ok: true, ...state }))
        .catch((error) => json(response, { ok: false, error: error.message }));
    }
    ...
    if (url.pathname === '/api/whatsapp/chats' && httpRequest.method === 'GET') {
      if (chatsCache.value && Date.now() - chatsCache.at < 60_000)
        return json(response, { ok: true, chats: chatsCache.value, cached: true });
      return withReady(response, json, (session) =>
        request(`/api/sessions/${encodeURIComponent(session.id)}/chats?limit=100`).then(
          async (chats) => {
            ...
            void warmPictures(
              session,
              resolved.slice(0, 20).map((chat) => chat.id),
            );
            json(response, {
              ok: true,
              chats: resolved,
              cached: false,
              source: hasUsableRecents(named) ? 'openwa' : snapshot ? 'snapshot' : 'openwa',
            });
          },
        ),
      );
    ...
    if (url.pathname === '/api/whatsapp/contacts' && httpRequest.method === 'GET') {
      ...
      return withReady(response, json, (session) =>
        request(`/api/sessions/${encodeURIComponent(session.id)}/contacts?limit=1000`).then(
          (contacts) => {
            const matches = (contacts || [])
              .filter((contact) =>
                `${contact.name || ''} ${contact.pushName || ''} ${contact.notify || ''} ${contact.id || ''}`
                  .toLowerCase()
                  .includes(query),
              )
              .slice(0, 20);
      ...
      return withReady(response, json, async (session) => {
        ...
        try {
          messages = await request(
            `/api/sessions/${encodeURIComponent(session.id)}/messages/${id}/history?limit=40`,
          );
        } catch (_) {
          messages = await request(
            `/api/sessions/${encodeURIComponent(session.id)}/messages?chatId=${id}&limit=40`,
          );
        }
    ...
    if (url.pathname === '/api/whatsapp/profile-pictures' && httpRequest.method === 'GET') {
      ...
      sessionState()
        .then(({ session }) => {
          if (session?.status === 'ready') void warmPictures(session, ids);
        })
        .catch(() => {});
    ...
    if (url.pathname === '/api/whatsapp/send' && httpRequest.method === 'POST') {
      try {
        const {
          ...
          attachments = [],
          approvalToken,
        } = JSON.parse(await readBody(httpRequest));
        ...
        const normalizedAttachments = Array.isArray(attachments)
          ? attachments
              ...
              .filter(Boolean)
          : [];
        if (!chatId || (!trimmedText && !normalizedAttachments.length) || trimmedText.length > 4096)
          return json(response, { ok: false, error: 'Invalid message' });
        if (Array.isArray(attachments) && attachments.length !== normalizedAttachments.length)
          return json(response, { ok: false, error: 'One or more attachments are invalid' });
        const approvalPayload = {
          chatId: String(chatId),
          text: trimmedText,
          attachments: normalizedAttachments.map(({ name, mime, bytes }) => ({
            name,
            mime,
            bytes,
          })),
        ...
        return withReady(response, json, async (session) => {
          const sent = [];
          if (trimmedText && !normalizedAttachments.length)
            sent.push(
              await request(`/api/sessions/${encodeURIComponent(session.id)}/messages/send-text`, {
                method: 'POST',
                body: JSON.stringify({ chatId, text: trimmedText }),
              }),
            );
          for (let index = 0; index < normalizedAttachments.length; index += 1) {
            ...
            sent.push(
              await request(
                `/api/sessions/${encodeURIComponent(session.id)}/messages/send-document`,
                {
                  method: 'POST',
                  body: JSON.stringify({
                    chatId,
        ...
        return json(response, { ok: false, error: error?.message || 'Invalid message' });
      }
    }
    return false;
  }
