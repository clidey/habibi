...
const cleanMailText = (value) =>
  String(value || '')
    .replace(/\r\n/g, '\n')
    .replace(/^\s*\[cid:[^\]]+\].*$/gim, '')
    .replace(/<https?:\/\/[^>\s]+>/g, '')
    .replace(
      /^Note:\s*This email is received from an external sender[^\n]*(?:\n(?!\s*\n)[^\n]*)*/gim,
      '',
    )
    .replace(/^This message \(including any attachments\)[\s\S]*$/gim, '')
    .replace(/^This message and any attachments are confidential[\s\S]*$/gim, '')
    .replace(/^_{10,}\s*$/gm, '')
    .replace(/\n[ \t]*\n[ \t]*\n+/g, '\n\n')
    .trim();
...
const addressFrom = (value) =>
  String(value || '')
    ...
    ?.toLowerCase() || '';
const conversationSubject = (value) =>
  String(value || '')
...
function sectionHeaders(section) {
  ...
  if (boundary < 0)
    return {
      from: normalized.match(/^From:\s*(.+)$/im)?.[1]?.trim() || '',
      to: '',
      sent: '',
      body: '',
    };
...
function splitMailThread(body, latestFrom, ownEmail, fallbackTimestamp = 0, firstHtml = '') {
  ...
  const messages = sections
    .map((section, index) => {
      ...
      return {
        ...
        direction:
          sender && sender === String(ownEmail || '').toLowerCase() ? 'outgoing' : 'incoming',
        timestamp: Number.isFinite(parsedTime)
          ? parsedTime
          : Math.max(0, fallbackTimestamp - index),
      };
    })
  ...
  return messages.map((message, index) => ({
    ...message,
    forwardedFrom: /forwarded message/i.test(sections[index] || '')
      ? messages[index + 1]?.from || ''
      : '',
    forwardedTo: /forwarded message/i.test(sections[index] || '')
      ? messages[index + 1]?.to || ''
      : '',
  }));
}
...
function sentMailboxPath(mailboxes) {
  return (
    mailboxes.find((mailbox) => {
      const special = Array.isArray(mailbox.specialUse)
        ? mailbox.specialUse.join(' ')
        : String(mailbox.specialUse || '');
      return (
        /\\Sent/i.test(special) || /^(sent|sent items|sent mail)$/i.test(String(mailbox.path || ''))
      );
    })?.path || ''
  );
}
...
function createMailService({ root, fs, spawn }) {
  ...
  const write = (value) => {
    ...
    fs.writeFileSync(configPath, JSON.stringify(value, null, 2), { mode: 0o600 });
  };
  ...
  const accountsFrom = (config) => {
    ...
    const legacy = Object.entries(PROVIDERS).flatMap(([provider]) =>
      config[provider]?.imap
        ? [{ id: provider, provider, imap: config[provider].imap, legacy: true }]
        : [],
    );
  ...
  const saveRefresh = (provider, token) =>
    command(
      'security',
      ['add-generic-password', '-U', '-s', keychainService, '-a', `refresh:${provider}`, '-w'],
      `${token}\n${token}\n`,
    );
  const getRefresh = async (provider) => {
  ...
  const saveSecret = (account, secret) =>
    command(
      'security',
      ['add-generic-password', '-U', '-s', keychainService, '-a', account, '-w'],
      `${secret}\n${secret}\n`,
    );
  ...
  const status = async () => {
    ...
    const accounts = await Promise.all(
      accountsFrom(config).map(async (account) => ({
        id: account.id,
        provider: account.provider,
        label: PROVIDERS[account.provider]?.label || account.provider,
        email: account.imap.email,
        connected:
          Boolean(await getSecret(`imap:${account.id}`)) ||
          (account.legacy && Boolean(await getSecret(`imap:${account.provider}`))),
        transport: 'imap',
      })),
    );
  ...
  const configureImap = async ({ provider, email, password, host, port = 993 }) => {
    ...
    const imap = {
      ...
      host: String(host || defaults).trim(),
    ...
    const client = new ImapFlow({
      ...
      port: imap.port,
      secure: true,
      auth: { user: imap.email, pass: String(password).replace(/\s+/g, '') },
      logger: false,
    });
    try {
      ...
      return {
        ok: false,
        error: `IMAP login failed: ${error.message || 'check IMAP access, server, and app password.'}`,
      };
    }
    const id = accountId(provider, imap.email);
    const config = read();
    config.accounts = config.accounts || {};
    if (String(config[provider]?.imap?.email || '').toLowerCase() === imap.email.toLowerCase())
      delete config[provider];
    config.accounts[id] = {
      provider,
      imap,
      createdAt: config.accounts[id]?.createdAt || new Date().toISOString(),
    };
    write(config);
    const saved = await saveSecret(`imap:${id}`, password.replace(/\s+/g, ''));
    return saved.ok
      ? { ok: true, account: { id, provider, email: imap.email } }
      : { ok: false, error: 'Could not save the app password in macOS Keychain.' };
  };
  const remove = async (id) => {
    const account = findAccount(id);
    if (!account) return { ok: false, error: 'Unknown mail account.' };
    const config = read();
    if (account.legacy) delete config[account.provider];
    else delete config.accounts?.[id];
    write(config);
  ...
  const threads = async (id) => {
    ...
    const password =
      (await getSecret(`imap:${id}`)) ||
      (account?.legacy ? await getSecret(`imap:${account.provider}`) : '');
    if (!imap || !password)
      return { ok: false, error: 'Connect this account with IMAP first.', threads: [] };
    const ImapFlow = getImapFlow();
    const client = new ImapFlow({
      host: imap.host,
      port: imap.port,
      secure: true,
      auth: { user: imap.email, pass: password },
      logger: false,
    });
    try {
      await client.connect();
      ...
      try {
        ...
        for await (const message of client.fetch(`${start}:*`, {
          ...
          rows.push({
            ...
            provider: account.provider,
            accountEmail: imap.email,
            subject: message.envelope?.subject || '(No subject)',
            from: message.envelope?.from?.[0]?.address || '',
            preview: '',
            timestamp: message.internalDate?.getTime() || 0,
      ...
      return { ok: false, error: error.message || 'Could not connect to IMAP.', threads: [] };
    } finally {
      await client.logout().catch(() => {});
    }
  };
  const searchCriteria = (plan) => {
    ...
    for (const term of terms)
      alternatives.push({ subject: term }, { from: term }, { to: term }, { body: term });
    const criteria =
      alternatives.length > 1 ? { or: alternatives } : alternatives[0] || { all: true };
  ...
  const scoreSearchResult = (thread, plan) => {
    ...
    const terms = [plan.from, plan.subject, ...(plan.terms || [])]
      .map((value) => String(value || '').toLowerCase())
      .filter(Boolean);
    const matches = terms.reduce((score, term) => score + (haystack.includes(term) ? 12 : 0), 0);
    const subjectBoost = (plan.terms || []).some((term) =>
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          threads: rows.sort(
            (left, right) =>
              scoreSearchResult(right, plan) - scoreSearchResult(left, plan) ||
      ...
      return {
        ok: false,
        error: error.message || 'Could not search this IMAP inbox.',
        threads: [],
      };
    } finally {
      await client.logout().catch(() => {});
  ...
  const search = async ({ query, provider = 'all', plan = {} }) => {
    const normalized = {
      terms: Array.isArray(plan.terms)
        ? plan.terms
            .map((value) => String(value).trim())
            .filter(Boolean)
            .slice(0, 8)
      ...
      after: String(plan.after || '').trim(),
      before: String(plan.before || '').trim(),
      unread: typeof plan.unread === 'boolean' ? plan.unread : null,
      source: plan.source === 'local-model' ? 'local-model' : 'rules',
    };
    const accounts =
      provider && provider !== 'all'
        ? [findAccount(provider)].filter(Boolean)
        : accountsFrom(read());
    if (!accounts.length) return { ok: true, threads: [], plan: normalized };
    const results = await Promise.all(
      accounts.map((account) => searchAccount(account.id, normalized)),
    );
    ...
    return {
      ...
      threads: results
        ...
        .sort(
          (left, right) =>
            scoreSearchResult(right, normalized) - scoreSearchResult(left, normalized) ||
            right.timestamp - left.timestamp,
        )
        .slice(0, 80),
      plan: normalized,
  ...
  const recent = async ({ provider, hours = 4 }) => {
    if (provider === 'all') {
      ...
      return {
        ok: true,
        threads: merged.filter((thread) => thread.timestamp >= Date.now() - windowMs),
      };
    }
    ...
    return {
      ...inbox,
      threads: (inbox.threads || []).filter((thread) => thread.timestamp >= since),
    };
  };
  const message = async ({ provider, uid }) => {
    ...
    const imap = account?.imap;
    const password =
      (await getSecret(`imap:${provider}`)) ||
      (account?.legacy ? await getSecret(`imap:${account.provider}`) : '');
    if (!imap || !password || !/^\d+$/.test(String(uid)))
      return { ok: false, error: 'Mail account or message is unavailable.' };
    const ImapFlow = getImapFlow();
    ...
    try {
      ...
      const messages = splitMailThread(
      ...
      if (sentPath) {
        ...
        try {
          ...
          for await (const sent of client.fetch(`${start}:*`, {
            envelope: true,
            source: true,
            internalDate: true,
          })) {
            if (conversationSubject(sent.envelope?.subject) !== conversationSubject(subject))
              continue;
            const sentParsed = await simpleParser(sent.source || Buffer.alloc(0), {
              skipImageLinks: true,
              skipTextToHtml: true,
            });
            const sentParts = splitMailThread(
              String(sentParsed.text || ''),
              sentParsed.from?.text || imap.email,
              imap.email,
              sent.internalDate?.getTime() || 0,
              safeEmailHtml(sentParsed.html),
            );
            ...
            if (sentParts[0])
              messages.push({
                ...sentParts[0],
                direction: 'outgoing',
                timestamp: sent.internalDate?.getTime() || sentParts[0].timestamp,
              });
          }
        } finally {
          sentLock.release();
      ...
      return { ok: false, error: error.message || 'Could not load the message.' };
    } finally {
      await client.logout().catch(() => {});
    }
  };
  const send = async ({ provider, to, subject, body }) => {
    ...
    const password =
      (await getSecret(`imap:${account.id}`)) ||
      (account.legacy ? await getSecret(`imap:${account.provider}`) : '');
    if (!password) return { ok: false, error: 'Connect this account with IMAP first.' };
    const transport = getNodemailer().createTransport({
      host: smtp.host,
      port: smtp.port,
      secure: true,
      auth: { user: imap.email, pass: password },
    ...
    try {
      ...
      return { ok: false, error: error.message || 'Could not send this email.' };
    } finally {
      transport.close();
    }
  };
  const authorize = (provider) => {
    ...
    if (!definition || !config?.clientId || !config?.clientSecret)
      return { ok: false, error: 'Configure this mail provider first.' };
    const state = crypto.randomUUID();
    pending.set(state, { provider, expiresAt: Date.now() + 10 * 60_000 });
    const params = new URLSearchParams({
      ...
      prompt: 'consent',
      state,
      scope: definition.scopes.join(provider === 'zoho' ? ',' : ' '),
    });
  ...
  const webUrl = ({ provider, subject, messageId }) => {
    ...
    if (kind === 'gmail') {
      const query = messageId
        ? `rfc822msgid:${String(messageId).replace(/[<>]/g, '')}`
  ...
  const callback = async ({ code, state }) => {
    ...
    if (!request || request.expiresAt < Date.now())
      return { ok: false, error: 'This connection request expired.' };
    const config = read()[request.provider];
    ...
    if (!response.ok || !token.refresh_token)
      return {
        ok: false,
        error:
          token.error_description || token.error || 'The provider did not return a refresh token.',
      };
    const stored = await saveRefresh(request.provider, token.refresh_token);
    return stored.ok
      ? { ok: true, provider: request.provider }
      : { ok: false, error: 'Could not save the token in macOS Keychain.' };
  };
  return {
    status,
    configure,
