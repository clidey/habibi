...
function jsonLines(fs, file, max = MAX_TRANSCRIPT_LINES) {
  ...
  return lines.slice(-max).flatMap((line) => {
    try {
...
function codexSession(fs, file) {
  ...
  const meta =
    ...
    firstJsonLine(fs, file).payload ||
    {};
  const id =
    meta.id || path.basename(file).match(/([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})/i)?.[1];
  ...
  const prompts = lines
    .flatMap((line) =>
      ...
      line.payload?.role === 'user'
        ? [textFromContent(line.payload.content)]
        : [],
    )
    .filter(Boolean);
  ...
  return {
    id,
    kind: 'codex',
    title: readablePrompt || 'Codex session',
    preview: readablePrompt || 'Local Codex transcript',
    cwd: meta.cwd || '',
    updatedAt,
    file,
  };
...
function claudeSession(fs, file) {
  ...
  if (!safeId(id)) return null;
  const prompts = lines
    .flatMap((line) => (line.type === 'user' ? [textFromContent(line.message?.content)] : []))
    .filter(Boolean);
  ...
  return {
    id,
    kind: 'claude',
    title: readablePrompt || 'Claude Code session',
    preview: readablePrompt || 'Local Claude transcript',
    cwd,
...
function transcript(fs, session) {
  const entries = jsonLines(fs, session.file).flatMap((line) => {
    if (session.kind === 'codex') {
      const message =
        line.type === 'response_item' && line.payload?.type === 'message' ? line.payload : null;
      if (message?.role === 'user' || message?.role === 'assistant') {
        const text = textFromContent(message.content);
        if (text) return [{ role: message.role, text, at: line.timestamp || '' }];
      }
      if (line.type === 'response_item' && line.payload?.type === 'function_call')
        return [
          {
            role: 'tool',
            text: `${line.payload.name || 'Tool'} ${trim(line.payload.arguments || '', 240)}`,
            at: line.timestamp || '',
          },
        ];
      return [];
    ...
    return text ? [{ role: line.type, text, at: line.timestamp || '' }] : [];
  });
...
function createAgentSessionsPlugin({ fs, home = process.env.HOME || '' }) {
  ...
  const all = () => {
    const codex = walk(fs, codexRoot, { include: (file) => file.endsWith('.jsonl') })
      .map((file) => codexSession(fs, file))
      .filter(Boolean);
    const claude = walk(fs, claudeRoot, {
      include: (file) =>
        file.endsWith('.jsonl') && !file.includes(`${path.sep}subagents${path.sep}`),
    })
      .map((file) => claudeSession(fs, file))
      .filter(Boolean);
    return [...codex, ...claude]
  ...
  return {
    list({ kind = '', query = '' } = {}) {
      ...
      const sessions = all().filter(
        (session) =>
          (!normalizedKind || session.kind === normalizedKind) &&
          (!needle ||
            `${session.title} ${session.preview} ${session.cwd}`.toLowerCase().includes(needle)),
      );
      return { ok: true, sessions: sessions.map(({ file, ...session }) => session) };
    },
    detail({ id, kind }) {
      if (!safeId(id) || !['codex', 'claude'].includes(kind))
        return { ok: false, error: 'That local agent session is not available.' };
      ...
      return {
        ok: true,
        session: (({ file, ...value }) => value)(session),
        transcript: transcript(fs, session),
      };
    },
  };
}
