...
const manifest = Object.freeze({
  ...
  tools: [
    {
      name: 'list_resources',
      description: 'List cluster resources in a context and optional namespace',
      readOnly: true,
    },
    {
      name: 'inspect_resource',
      description: 'Read structured workload, pod, service, or event details',
      readOnly: true,
    },
    { name: 'read_logs', description: 'Read a bounded tail of pod logs', readOnly: true },
    {
      name: 'diagnose',
      description:
        'Investigate a workload by sequencing safe cluster reads and summarizing evidence',
      readOnly: true,
    },
  ],
});
...
function createKubernetesPlugin({ llmService }) {
  async function plan({ request }) {
    ...
    if (!configured.configured || !['ollama', 'lmstudio'].includes(configured.provider))
      return {
        ok: false,
        error:
          'Configure a local model to interpret Kubernetes requests, or use a kubectl-style command.',
      };
    ...
    try {
      ...
      return typeof value.query === 'string'
        ...
        : { ok: false, error: 'The local model did not return a Kubernetes query.' };
    } catch (_) {
      return { ok: false, error: 'The local model returned an unreadable Kubernetes plan.' };
    }
  }
  const words = (text) =>
    new Set(
  ...
  const score = (requestWords, item) =>
    [...requestWords].reduce(
      (total, word) =>
        total + (`${item.name} ${item.namespace}`.toLowerCase().includes(word) ? 1 : 0),
      0,
    );
  async function diagnose({ request, context, namespace, tools }) {
    ...
    trace.push({
      tool: 'list_resources',
      detail: `Inspected pods, deployments, and services in ${overview.namespace || 'all namespaces'}.`,
    });
    const requestWords = words(request);
    const candidates = overview.resources
      .flatMap((resource) =>
        (resource.items || []).map((item) => ({ ...item, kind: resource.kind })),
      )
      .map((item) => ({ ...item, score: score(requestWords, item) }))
      .sort((left, right) => right.score - left.score);
    const target = candidates.find((item) => item.score > 0);
    if (!target) {
      const namespaces = [
        ...new Set(candidates.map((item) => item.namespace).filter(Boolean)),
      ].slice(0, 8);
      return {
        ok: true,
        clarification: true,
        summary: namespaces.length
          ? `I could not identify a matching workload yet. I found resources in ${namespaces.join(', ')}. Which namespace, workload, or pod should I inspect?`
          : 'I found no pods, deployments, or services in this scope. Which context or namespace should I inspect?',
        trace,
        candidates: [],
        evidence: [],
      };
    }
    const detailResult = await tools.detail({
      kind: target.kind,
      name: target.name,
    ...
    trace.push({
      ...
      detail: `Inspected ${detail.kind}/${detail.name} in ${detail.namespace || 'the cluster'}.`,
    });
    const logPod = detail.kind === 'pods' ? detail : detail.relatedPods?.[0];
    let logs = '';
    if (logPod?.name && logPod?.namespace) {
      const logResult = await tools.logs({
        pod: logPod.name,
        namespace: logPod.namespace,
        context: overview.context,
      });
      if (logResult.ok) {
        logs = String(logResult.output || '').slice(-12_000);
        trace.push({
          tool: 'read_logs',
          detail: `Read the latest bounded log tail from ${logPod.name}.`,
        });
      }
    }
    const evidence = [
      ...
      ...(detail.conditions || []).map(
        (item) =>
          `Condition ${item.type}: ${item.status}${item.reason ? ` (${item.reason})` : ''}${item.message ? ` — ${item.message}` : ''}`,
      ),
      ...(detail.containers || []).map(
        (item) =>
          `Container ${item.name}: ${item.state}, ready=${item.ready}, restarts=${item.restarts}`,
      ),
      ...(logs ? [`Recent logs:\n${logs}`] : []),
    ];
    const configured = await llmService.configured();
    ...
    if (configured.configured && ['ollama', 'lmstudio'].includes(configured.provider)) {
      const answer = await llmService.complete({
        systemPrompt:
          'You are the Kubernetes plugin diagnosis summarizer. Use only the supplied evidence. State what is healthy, what looks wrong, and the next safe inspection step. Do not claim certainty beyond the evidence. Do not suggest write commands. Keep it under 180 words.',
    ...
    return {
      ok: true,
      target: { kind: detail.kind, name: detail.name, namespace: detail.namespace },
      summary,
      trace,
      evidence,
      logs,
