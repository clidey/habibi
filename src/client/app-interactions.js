...
export function installAppInteractions(deps) {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const selected = Math.max(
        0,
        choices.findIndex((button) => button.classList.contains('selected')),
      );
      const select = (choice) => {
      ...
      if (event.key === 'Escape') {
        (
          choices.find((button) => button.dataset.choice === 'cancel') ||
          confirmation.querySelector('.back-button')
        )?.click();
        consume();
      } else if (['ArrowLeft', 'ArrowUp'].includes(event.key)) {
        select(choices[(selected - 1 + choices.length) % choices.length]);
        consume();
      } else if (['ArrowRight', 'ArrowDown'].includes(event.key)) {
        select(choices[(selected + 1) % choices.length]);
        consume();
      } else if (event.key === 'Enter') {
        (choices.find((button) => button.classList.contains('selected')) || choices[0])?.click();
        consume();
      }
    },
    true,
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      if (
        ...
        !target.closest('.content, #default-view, #results-view')
      )
        return;
      event.preventDefault();
      const direction = event.key === 'ArrowDown' ? 1 : -1;
      resultsView.classList.contains('hidden')
        ? keyboard.navigateKeyboard(direction)
        : keyboard.navigateResults(direction, getMode() !== 'whatsapp');
    },
    true,
  );
  input.addEventListener('input', (event) => {
    ...
    searchTimer = setTimeout(() => {
      ...
      track('habibi.search.submitted', {
        surface: 'launcher',
        query_length_bucket: lengthBucket(query),
        query_word_count_bucket: countBucket(query.split(/\s+/).filter(Boolean).length),
        app_type: 'native',
        app_version: '0.1.0',
      });
      renderSearch(query);
  ...
  input.addEventListener('keydown', (event) => {
    if (
      event.key === 'Escape' &&
      !document.querySelector('.system-action-confirm') &&
      !document.querySelector('#quick-preview')
    ) {
      event.preventDefault();
      return onDismiss();
    }
    if (getMode() === 'kubernetes' && event.key === 'Enter') {
      event.preventDefault();
      return kubernetes.runQuery();
    }
    ...
    if (event.key === 'Enter' && !resultsView.classList.contains('hidden')) {
      event.preventDefault();
      resultActions.activate(
        document.querySelector('.result.selected') || document.querySelector('.result'),
      );
    }
  });
  document.addEventListener('keydown', (event) => {
    ...
    if (
      ...
      event.preventDefault();
      return keyboard.navigateKeyboard(event.key === 'ArrowDown' ? 1 : -1);
    }
    if (!activeResult) return;
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      event.preventDefault();
      return keyboard.navigateResults(event.key === 'ArrowDown' ? 1 : -1);
    }
    if (event.code === 'Space' && activeResult.dataset.path) {
      event.preventDefault();
      event.stopPropagation();
      return openFilePreview(
        decodeURIComponent(activeResult.dataset.path),
        activeResult.dataset.title,
      );
    }
    if (event.key === 'Enter') {
      event.preventDefault();
      resultActions.activate(activeResult);
    }
  });
  return keyboard;
}
