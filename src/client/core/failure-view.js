 ...
 * caller's own copy.
 */
const looksInternal = message => /(^Error:|\bat \S+:\d+:\d+|\/Users\/|\/var\/|\/etc\/|ENOENT|ECONNREFUSED|ECONNRESET|EACCES|ETIMEDOUT|errno|node_modules|TypeError|ReferenceError)/i.test(message);
 ...
 */
export const renderFailure = (container, error, { fallback = 'Something went wrong.', retry } = {}) => {
  if (!container) return;
  const message = categorizeError(error, fallback);
  setHtml(container, `<div class="local-files-empty">${escapeHtml(message)}${retry ? '<button type="button" class="link-button failure-retry">Try again</button>' : ''}</div>`);
