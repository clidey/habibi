...
export function installClipboardImageBridge({ notify, onFiles }) {
  async function receive(payload) {
    ...
    const files = [new File([new Blob([bytes], { type:mime })], 'Pasted screenshot.png', { type:mime })];
  ...
  window.__habibiReceiveNativeClipboardImage = payload => receive(payload).catch(() => notify('Habibi could not read an image from the clipboard.'));
  window.__habibiNativePasteImage = () => { if (!requestNativeClipboardImage()) notify('Habibi could not read an image from the clipboard.'); };
}
