...
export function createSkillsFeature({
  ...
  function showSkills() {
    ...
    const skills = [
      ['⌁', 'files', 'Files', 'Native index · Metadata read'],
      ['□', 'calendar', 'Calendar', 'EventKit · Read & write'],
      ['M', 'gmail', 'Mail', 'Gmail + Zoho IMAP · Local accounts'],
      ['◔', 'whatsapp', 'WhatsApp', 'OpenWA · Localhost only'],
      ['◉', 'files', 'Browser', 'Extension · Tabs & history'],
      ['✣', 'agents', 'Agent Dock', 'Local Codex & Claude processes'],
    ];
    const render = (imported) => {
      ...
      resultsView
        .querySelectorAll('[data-imported-skill]')
        .forEach(
          (button) => (button.onclick = () => showImportedSkill(button.dataset.importedSkill)),
        );
      refreshIcons();
    };
    fetch('/api/agent-skills')
      .then((response) => response.json())
      .then((data) => render(data.ok ? data.skills || [] : []))
      .catch(() => render([]));
  }
  function showImportedSkill(id) {
    onOpen('imported-skill');
    count.textContent = 'Inspecting imported skill…';
    setHtml(
      resultsView,
      `<div class="result-header conversation-mode"><button class="back-button" id="back-imported-skills">${icon('arrow-left')} Skills</button><span class="verified">● review before run</span></div><div class="loading-state"><span class="spinner"></span> Inspecting this local capability…</div>`,
    );
    document.querySelector('#back-imported-skills').onclick = showSkills;
    fetch('/api/agent-skills/preview', {
      method: 'POST',
      ...
      .then((data) => {
        ...
        const run = async ({ toolName, toolInput }) => {
          ...
          const button = document.querySelector('#run-imported-skill');
          if (button) {
            button.disabled = true;
            setHtml(button, '<span class="mini-spinner"></span> Starting…');
          }
          const result = await fetch('/api/agent-skills/execute', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id, toolName, toolInput, approvalToken }),
          }).then((response) => response.json());
          if (!result.ok) {
            if (button) {
            ...
            return notify(result.error || 'Could not run that skill.');
          }
          if (result.result) {
            const output = document.querySelector('#imported-skill-output');
            setHtml(
              output,
              `<pre>${escapeHtml(JSON.stringify(result.result, null, 2).slice(0, 12_000))}</pre>`,
            );
            notify('MCP tool completed');
          } else {
            notify(
              `${skill.source === 'codex' ? 'Codex' : 'Claude Code'} opened with your approved request`,
            );
            onHome();
          }
        };
        if (skill.kind === 'mcp-server') {
          ...
          const renderMcpReview = (selectedTool = tools[0]?.name || '', pendingInput) => {
            ...
            document.querySelector('#run-imported-skill').onclick = () => {
              ...
              try {
                ...
                return notify('Tool input must be valid JSON.');
              }
              const tool = tools.find((candidate) => candidate.name === toolName);
              if (tool && tool.readOnly === false)
                return renderMcpWriteConfirm(tool, toolInput, rawInput);
          ...
          const renderMcpWriteConfirm = (tool, toolInput, rawInput) => {
            ...
            resultsView.querySelectorAll('.confirmation-choice').forEach(
              (button) =>
                (button.onclick = () => {
                  document.querySelector('.system-action-confirm').dataset.confirmChoice =
                    button.dataset.choice;
                  resultsView
                    .querySelectorAll('.confirmation-choice')
                    .forEach((choice) => choice.classList.toggle('selected', choice === button));
                  if (button.dataset.choice === 'cancel') back();
                }),
            );
            document.querySelector('#confirm-mcp-tool').onclick = () => {
              renderMcpReview(tool.name, rawInput);
              run({ toolName: tool.name, toolInput });
            };
            refreshIcons();
          };
          renderMcpReview();
          ...
          document.querySelector('#back-imported-skills').onclick = showSkills;
          document.querySelector('#run-imported-skill').onclick = () =>
            run({ toolInput: document.querySelector('#imported-skill-request').value });
        }
        refreshIcons();
      })
      .catch((error) => {
        renderFailure(resultsView, error, {
          fallback: 'Could not inspect this skill.',
          retry: () => showImportedSkill(id),
        });
      });
  }
