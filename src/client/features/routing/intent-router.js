...
export function createIntentRouter({ input, defaultView, resultsView, count, notify, kubernetes, calendar, mail, whatsapp, onChat, onHome }) {
...
function routeAppIntent(intent) {
  ...
  if (intent.kind === 'calendar') return calendar.showDraft(calendarDraftFromText(intent.source || ''));
  ...
  fetch('/api/whatsapp/chats').then(response => response.json()).then(data => {
    ...
    if (resolved.chat) {
      ...
      if (resolved.instruction) whatsapp.draftMessage(resolved.chat, resolved.instruction, intent.original);
      return;
    }
    whatsapp.showChats();
    setTimeout(() => { input.value = intent.target; whatsapp.filter(intent.target); input.focus(); }, 120);
  }).catch(() => notify('Could not read your WhatsApp chats'));
}
...
function openAgentBrowserSearch(route) {
  ...
  return { openBrowser:openAgentBrowserSearch, parse:parseAppIntent, route:routeAppIntent, showAgentic:showAgenticMessage };
}
