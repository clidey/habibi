...
export function createResultActions(deps) {
  ...
  async function showAction(type, title, filePath) {
    ...
    if (type === 'file' && !filePath) {
      input.focus();
      return notify('Type a filename to search your local Spotlight index');
    }
    if (type === 'file' && filePath) {
      try {
    ...
    setHtml(
      resultsView,
      `<div class="result-header conversation-mode"><button class="back-button" id="back-action">${icon('arrow-left')} Habibi</button><span>External actions need approval</span></div><div class="compose"><span class="compose-label">Action unavailable</span><h2>${escapeHtml(title || 'Habibi')}</h2><p>This action is not available yet.</p><div class="compose-actions"><button class="secondary" id="cancel">Back</button></div></div>`,
    );
  ...
  function activate(result) {
    if (!result) return;
    track('habibi.result.opened', {
      result_type: String(result.dataset.type || 'unknown').slice(0, 32),
      surface: getMode() || 'search',
      app_type: 'native',
      app_version: '0.1.0',
    });
    if (result.dataset.mailThread)
      return mail.showThread(result.dataset.mailThread, result.dataset.mailProvider);
    ...
    if (result.dataset.type === 'app' && result.dataset.path)
      return platformActions.openApp(result);
    if (result.dataset.type === 'kubernetes') return kubernetes.show(input.value);
    if (['codex', 'claude'].includes(result.dataset.type))
      return agentSessions.showSessions(result.dataset.type);
    if (result.dataset.type === 'system') {
      if (result.dataset.systemAction === 'quitApps') return platformActions.showRunning('quit');
      if (result.dataset.systemAction === 'forceQuitApps')
        return platformActions.showRunning('force');
    ...
    showAction(
      ...
      result.dataset.path && decodeURIComponent(result.dataset.path),
    );
  }
