...
export function calendarDraftFromText(text = '') {
  ...
  const time = lower.match(
    /\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\s*(?:-|to|–|—)\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\b/,
  );
  const hour = (value, meridiem) => {
    let result = Number(value);
    if (meridiem === 'pm' && result < 12) result += 12;
    if (meridiem === 'am' && result === 12) result = 0;
    return result;
  };
  ...
  const person = String(text)
    .match(
      /\b(?:meeting|meet|call|lunch|dinner)\s+(?:with\s+)?([^,?.]+?)(?:\s+(?:next|tomorrow|on)\b|$)/i,
    )?.[1]
    ?.trim();
