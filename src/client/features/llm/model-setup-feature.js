...
import { installModelSetupKeyboard } from './model-setup-keyboard.js';

/** Owns provider discovery, model selection, credential setup, and validation. */
export function createModelSetupFeature({
  defaultView,
  resultsView,
  count,
  onBack,
  ...
  function show({ afterConfigured } = {}) {
    ...
    setHtml(
      resultsView,
      `<div class="result-header conversation-mode"><button class="back-button" id="back-setup">${icon('arrow-left')} Habibi</button><span class="verified">● private by design</span></div><section class="provider-setup"><div class="chat-title"><span class="icon agents">${icon('sparkles')}</span><span><b>Connect a model</b><small>Pick a provider first—then we’ll only ask for what it needs.</small></span></div><div class="provider-options" role="radiogroup">${Object.entries(
        llmProviders,
      )
        .map(
          ([id, provider]) =>
            `<button class="provider-option" data-provider="${id}" role="radio" aria-checked="false"><span><b>${provider.label}</b><small>${provider.description}</small></span><em>${provider.kind === 'local' ? 'LOCAL' : 'YOUR KEY'}</em><i>${icon('chevron-right')}</i></button>`,
        )
        .join('')}</div><div id="provider-detail" aria-live="polite"></div></section>`,
    );
    let selected = 'ollama';
    ...
    const select = (providerId) => {
        ...
        setHtml(
          ...
          matching.length
            ? matching
                .map(
                  (model, index) =>
                    `<button class="${model === activeModel ? 'active-model-option' : ''}" role="option" data-model="${escapeHtml(model)}" aria-selected="${model === activeModel || index === 0}"><span>${escapeHtml(model)}</span>${model === activeModel ? '<em>Active</em>' : ''}</button>`,
                )
                .join('')
            : '<span class="model-empty">Type any installed model name</span>',
        );
        modelMenu.querySelectorAll('[data-model]').forEach(
          (button) =>
            (button.onclick = () => {
      ...
      document.querySelector('#llm-model-trigger').onclick = () =>
      ...
      modelInput.addEventListener('keydown', (event) => {
        if (event.key === 'ArrowDown') {
          ...
          document.querySelector('#llm-endpoint, #llm-api-key, #save-llm')?.focus();
        }
        if (event.key === 'Escape') {
          event.preventDefault();
          closeModelMenu();
        }
        if (event.key === 'Enter' && !modelMenu.classList.contains('hidden')) {
          event.preventDefault();
          closeModelMenu();
          document.querySelector('#llm-endpoint, #llm-api-key, #save-llm')?.focus();
        }
      ...
      const loadModels = async ({ useTypedKey = false } = {}) => {
        ...
        const response = await fetch('/api/llm/models', {
          ...
          body: JSON.stringify({ provider: selectedProviderId, endpoint, apiKey }),
        });
        const data = await response.json();
        // Ignore a response from a provider panel the user has already left.
        if (
          selectionVersion !== providerSelectionVersion ||
          selectedProviderId !== selected ||
          !modelInput.isConnected
        )
          return;
        if (data.models?.length) {
          availableModels = data.models;
          if (provider.kind === 'local' && modelInput.value === provider.model)
            modelInput.value = data.models[0];
          renderModels();
          document.querySelector('#llm-setup-message').textContent =
            `${data.models.length} available model${data.models.length === 1 ? '' : 's'} found — start typing to filter.`;
        } else if (provider.kind === 'local')
          document.querySelector('#llm-setup-message').textContent =
            'No models found yet. Start the local server, or type a model name.';
        else
          document.querySelector('#llm-setup-message').textContent =
            'Add or keep an API key to load available models, or type a model name.';
      };
      loadModels().catch(() => {});
    ...
    const save = async () => {
      ...
      const response = await fetch('/api/llm/configure', {
        ...
        body: JSON.stringify({
          ...
          endpoint:
            document.querySelector('#llm-endpoint')?.value || llmProviders[selected].endpoint,
          apiKey: document.querySelector('#llm-api-key')?.value,
        }),
      });
      const data = await response.json();
      if (!data.ok || !data.configured) {
        button.disabled = false;
        setHtml(button, 'Continue <kbd>↵</kbd>');
        message.textContent = data.error || 'Could not connect to that provider.';
        return;
      }
    ...
    fetch('/api/llm/status')
      ...
      .then((state) => {
        ...
        activeConfiguration = {
          ...
          model: state.model || llmProviders[state.provider].model,
        };
        const activeOption = document.querySelector(
          `.provider-option[data-provider="${state.provider}"]`,
        );
