...
export function createResultButton({ icon, chatTime, iconNames }) {
  return function resultButton(item, index) {
    ...
    const iconMarkup = item.appIcon ? `<img src="${safeImageSrc(item.appIcon)}" alt="" onerror="this.remove();this.closest('.app-icon').classList.add('icon-unavailable')" />${appFallback}` : canShowChatAvatar && item.avatar ? `<img src="${safeImageSrc(item.avatar)}" alt="" />` : canShowChatAvatar && item.initials ? `<span>${escapeHtml(item.initials)}</span>` : icon(isPdf ? 'file-text' : iconNames[item.icon]);
