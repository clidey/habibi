...
} = require('./src/core/http-security');
...
const skills = [...builtInSkills, kubernetesPlugin].sort((left, right) =>
...
process.on('unhandledRejection', (error) =>
  console.error('[Habibi connector rejection]', error?.message || error),
);
process.on('uncaughtException', (error) => {
  if (fatalStartupCodes.has(error?.code)) {
    console.error(
      `[Habibi] cannot listen on ${HOST}:${PORT}: ${error.code}. Another Habibi or a development server is probably already running.`,
    );
    process.exit(1);
  }
...
const server = http.createServer(async (request, response) => {
  ...
  if (!isTrustedLocalRequest(request))
    return response
      ...
      .end('{"ok":false,"error":"Local requests only"}');
  ...
  if (
    await whatsappService.handle({
      request,
      response,
      url,
      json,
      safeJsonValue,
      requiresApproval: (payload) => approvals.consume(payload),
    })
  )
    return;
  if (url.pathname === '/api/analytics/capture' && request.method === 'POST')
  ...
  if (url.pathname === '/api/mail/status' && request.method === 'GET')
    return json(response, await mailService.status());
  if (url.pathname === '/api/mail/open' && request.method === 'POST')
    return readJson(request, response, (body) => {
      const target = mailService.webUrl(body);
      if (!target)
        return json(response, { ok: false, error: 'That mail provider cannot be opened.' });
      spawn('open', [target], { detached: true, stdio: 'ignore' }).unref();
      return json(response, { ok: true, provider: body.provider });
    });
  if (url.pathname === '/api/mail/configure' && request.method === 'POST')
    return readJson(request, response, (body) => json(response, mailService.configure(body)));
  if (url.pathname === '/api/mail/imap' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await mailService.configureImap(body)),
    );
  if (url.pathname === '/api/mail/remove' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await mailService.remove(body.provider)),
    );
  if (url.pathname === '/api/mail/threads' && request.method === 'GET')
    return json(response, await mailService.threads(url.searchParams.get('provider')));
  ...
  if (url.pathname === '/api/mail/recent' && request.method === 'GET')
    return json(
      response,
      await mailService.recent({
        provider: url.searchParams.get('provider'),
        hours: url.searchParams.get('hours'),
      }),
    );
  if (url.pathname === '/api/mail/message' && request.method === 'GET')
    return json(
      response,
  ...
  if (url.pathname === '/api/mail/authorize' && request.method === 'POST')
    return readJson(request, response, (body) =>
      json(response, mailService.authorize(body.provider)),
    );
  if (url.pathname === '/api/mail/oauth/callback' && request.method === 'GET') {
    const result = await mailService.callback({
      code: url.searchParams.get('code'),
      state: url.searchParams.get('state'),
    });
    response.writeHead(result.ok ? 200 : 400, { 'Content-Type': 'text/html; charset=utf-8' });
    return response.end(
      `<title>Habibi Mail</title><body style="font-family:-apple-system,sans-serif;padding:40px">${result.ok ? 'Mail connected. You can close this tab and return to Habibi.' : `Mail connection failed: ${String(result.error || 'Unknown error').replace(/</g, '&lt;')}`}</body>`,
    );
  }
  if (url.pathname === '/api/mail/send' && request.method === 'POST')
    return readJson(request, response, async (body) => {
      ...
      if (!approvals.consume({ token: body.approvalToken, action: 'mail.send', payload }))
        return json(response, { ok: false, error: 'Sending needs explicit approval' });
  ...
  if (url.pathname === '/api/approvals' && request.method === 'POST')
    return readJson(request, response, (body) => {
      ...
      if (
        !/^(?:whatsapp\.send|mail\.send|calendar\.(?:create|update)|agent-skill\.execute|running-app\.(?:quit|force)|system\.(?:sleep|restart|shutdown|lock|darkMode|emptyTrash))$/.test(
          action,
        )
      )
        return json(response, { ok: false, error: 'Unsupported approval action' });
  ...
  if (url.pathname === '/api/llm/status' && request.method === 'GET')
    return json(response, await llmService.configured());
  if (url.pathname === '/api/agent-skills' && request.method === 'GET')
    return json(response, { ok: true, skills: importedSkills.list() });
  if (url.pathname === '/api/agent-skills/preview' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await importedSkills.preview(String(body.id || ''))),
    );
  if (url.pathname === '/api/agent-skills/execute' && request.method === 'POST')
    return readJson(request, response, async (body) => {
      ...
      if (
        !approvals.consume({
          ...
          action: 'agent-skill.execute',
          payload: { id, toolName: toolName ?? null, toolInput: body.toolInput ?? null },
        })
      )
        return json(response, {
          ok: false,
          error: 'Running an imported skill needs explicit approval.',
        });
      return json(
        response,
        await importedSkills.execute({ id, toolName, toolInput: body.toolInput }),
      );
    });
  if (url.pathname === '/api/mcp/servers' && request.method === 'GET')
    return json(response, { ok: true, servers: mcpBridge.list() });
  if (url.pathname === '/api/mcp/tools' && request.method === 'GET')
    return json(response, await mcpBridge.discover(url.searchParams.get('server')));
  if (url.pathname === '/api/llm/models' && request.method === 'GET')
    return json(
      response,
      await llmService.models({
        provider: url.searchParams.get('provider'),
        endpoint: url.searchParams.get('endpoint'),
      }),
    );
  if (url.pathname === '/api/llm/models' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(
        response,
        await llmService.models({
  ...
  if (url.pathname === '/api/llm/configure' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await llmService.configure(body)),
    );
  ...
  if (url.pathname === '/api/agent/files/investigate' && request.method === 'POST')
    return readJson(request, response, async (body) => {
      ...
      if (plan.phase === 'not_applicable')
        return json(response, { ok: true, phase: 'not_applicable' });
      if (plan.phase === 'clarify')
        return json(response, {
          ...
          trace: [
            { tool: 'Local file planner', detail: 'Need one detail before searching accurately.' },
          ],
        });
      const window = relativeFileWindow(
        history.filter((turn) => turn.role === 'user').at(-1)?.text || '',
      );
      const searches = await Promise.all(
        (plan.queries || []).slice(0, 3).map((query) => findLocalFiles(query, 10, { window })),
      );
      const seen = new Set();
      ...
      candidates.sort(
        (a, b) => (order.get(a.path) ?? 999) - (order.get(b.path) ?? 999) || b.score - a.score,
      );
      const files = candidates.slice(0, 8).map(({ score, ...file }) => file);
      const summary = files.length
        ? ranked.ids?.length
          ? ranked.summary ||
            `I found ${files.length} likely local file${files.length === 1 ? '' : 's'} to review.`
          : `I found ${files.length} likely local file${files.length === 1 ? '' : 's'} to review.`
        : ranked.summary ||
          'I did not find a close match yet. What country, year, or document name should I try?';
      return json(response, {
        ok: true,
        phase: 'results',
        summary,
        ...
        trace: [
          {
            tool: 'Local file planner',
            detail: `Prepared ${(plan.queries || []).length} focused filename search${(plan.queries || []).length === 1 ? '' : 'es'}.`,
          },
          {
            tool: 'Filename search',
            detail: files.length
              ? `Found ${files.length} candidate${files.length === 1 ? '' : 's'} locally.`
              : 'No close filename candidates yet.',
          },
          {
            tool: 'Local ranker',
            detail: files.length
              ? 'Ranked candidates using names and locations.'
              : 'Suggested the next refinement.',
          },
        ],
      });
    });
  if (url.pathname === '/api/agent/route' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(
        response,
        await llmService.route({
  ...
  if (url.pathname === '/api/open-app' && request.method === 'POST')
    return readJson(request, response, (body) => {
      ...
      if (
        !target.endsWith('.app') ||
        !allowedRoots.some((prefix) => target.startsWith(prefix)) ||
        !fs.existsSync(target)
      )
  ...
  if (url.pathname === '/api/app-icon' && request.method === 'GET') {
    ...
    const icon = fs.existsSync(resourceDir)
      ? fs.readdirSync(resourceDir).find((file) => file.toLowerCase().endsWith('.icns'))
      : null;
    if (!icon) return response.writeHead(404).end('Not found');
    fs.mkdirSync(cacheDir, { recursive: true });
    const converter = spawn('sips', [
      '-s',
      'format',
      'png',
      path.join(resourceDir, icon),
      '--out',
      cacheFile,
    ]);
    converter.on('error', () => response.writeHead(404).end('Not found'));
    converter.on('close', (code) =>
      code === 0 ? sendIcon() : response.writeHead(404).end('Not found'),
    );
    return;
  }
  ...
  if (url.pathname === '/api/apps' && request.method === 'GET')
    return applications(url.searchParams.get('q') || '').then((apps) =>
      json(response, { ok: true, apps }),
    );
  if (url.pathname === '/api/kubernetes/query' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(
        response,
        await runKubernetesReadQuery(
          String(body.query || ''),
          String(body.context || ''),
  ...
  if (url.pathname === '/api/kubernetes/plan' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(
        response,
        await planKubernetesNaturalQuery(
          String(body.query || ''),
          String(body.context || ''),
          String(body.namespace || ''),
        ),
  ...
  if (url.pathname === '/api/kubernetes/diagnose' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(
        response,
        await diagnoseKubernetes(
          String(body.query || ''),
          String(body.context || ''),
          String(body.namespace || ''),
        ),
  ...
  if (url.pathname === '/api/kubernetes/resources' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await kubernetesResourceList(body)),
    );
  if (url.pathname === '/api/kubernetes/detail' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await kubernetesResourceDetail(body)),
    );
  if (url.pathname === '/api/kubernetes/logs' && request.method === 'POST')
    return readJson(request, response, async (body) =>
      json(response, await kubernetesPodLogs(body)),
    );
  if (url.pathname === '/api/kubernetes/overview' && request.method === 'GET')
    return json(
      response,
      await kubernetesOverview(
        url.searchParams.get('context') || '',
        url.searchParams.get('namespace') || '',
      ),
    );
  if (url.pathname === '/api/running-apps' && request.method === 'GET')
    return runningApplications()
      .then((apps) => json(response, { ok: true, apps }))
      .catch(() => json(response, { ok: false, apps: [] }));
  if (url.pathname === '/api/running-apps/action' && request.method === 'POST')
    return readJson(request, response, async (body) => {
      ...
      if (!mode || !app || !pids.length)
        return json(response, { ok: false, error: 'Choose an open application first.' });
      if (!approvals.consume({ token: body.approvalToken, action: `running-app.${mode}`, payload }))
        return json(response, { ok: false, error: 'Quitting an app needs explicit approval.' });
      const current = await runningApplications();
      const target = current.find(
        (item) =>
          item.name === app &&
          item.pids.length === pids.length &&
          item.pids.every((pid) => pids.includes(pid)),
      );
      if (!target)
        return json(response, {
          ok: false,
          error: 'That app is no longer running. Refresh the list and try again.',
        });
      let stopped = 0;
      for (const pid of pids) {
        try {
          ...
          /* A process may exit between listing and action. */
        }
      }
  ...
  if (url.pathname === '/api/system/action' && request.method === 'POST')
    return readJson(request, response, (body) => {
      ...
      if (openActions[action]) {
        const [command, args] = openActions[action];
        spawn(command, args, { detached: true, stdio: 'ignore' }).unref();
        return json(response, { ok: true });
      }
      if (!['sleep', 'restart', 'shutdown', 'lock', 'darkMode', 'emptyTrash'].includes(action))
        return json(response, { ok: false, error: 'Unknown system action' });
      if (
        !approvals.consume({
          token: body.approvalToken,
        ...
        return json(response, { ok: false, error: 'This system action needs explicit approval.' });
      const commands = {
        ...
        emptyTrash: ['osascript', ['-e', 'tell application "Finder" to empty the trash']],
      };
  ...
  if (url.pathname === '/api/quick-look' && request.method === 'POST')
    return readJson(request, response, (body) => {
      ...
      quickLookProcess.on('error', () => {
        quickLookProcess = null;
      });
      quickLookProcess.on('close', () => {
        quickLookProcess = null;
      });
  ...
  if (url.pathname === '/api/files') {
    ...
    return findLocalFiles(query, 8, { window: relativeFileWindow(query) }).then((files) =>
      json(response, files),
    );
    return;
  }
  if (url.pathname === '/api/calendars' && request.method === 'GET') {
    return runJxa(
      'function run(argv){ return JSON.stringify(Application("Calendar").calendars.name()); }',
      [],
      (error, output) => {
        if (error) return json(response, { ok: false, calendars: [] });
        try {
          json(response, { ok: true, calendars: JSON.parse(output) });
        } catch (_) {
          json(response, { ok: false, calendars: [] });
        }
      },
    );
  }
  if (url.pathname === '/api/calendar/event' && request.method === 'POST')
    return readJson(request, response, (event) => {
      const payload = {
      ...
      if (!approvals.consume({ token: event.approvalToken, action: 'calendar.create', payload }))
        return json(response, { ok: false, error: 'Calendar creation needs explicit approval' });
      const script =
        'function run(argv){ const app=Application("Calendar"); const calendar=app.calendars.byName(argv[0]); const item=app.Event({summary:argv[1],startDate:new Date(argv[2]),endDate:new Date(argv[3])}); calendar.events.push(item); }';
      return runJxa(
        script,
        [payload.calendar, payload.title, payload.start, payload.end],
        (error) => json(response, { ok: !error }),
      );
    });
  ...
  if (url.pathname === '/api/calendar/event/update' && request.method === 'POST')
    return readJson(request, response, (event) => {
      ...
      if (!approvals.consume({ token: event.approvalToken, action: 'calendar.update', payload }))
        return json(response, { ok: false, error: 'Calendar changes need explicit approval' });
      const script =
        'function run(argv){ const app=Application("Calendar"); const calendar=app.calendars.whose({name:argv[0]})[0]; const item=calendar.events.byId(argv[1]); item.summary=argv[2]; item.startDate=new Date(argv[3]); item.endDate=new Date(argv[4]); }';
      return runJxa(
        script,
        [payload.calendar, payload.id, payload.title, payload.start, payload.end],
        (error) => json(response, { ok: !error }),
      );
    });
  if (url.pathname === '/api/agents' && request.method === 'GET') {
    ...
    let output = '';
    processes.stdout.on('data', (chunk) => {
      output += chunk;
    });
    ...
    processes.on('close', () => {
      ...
      Promise.all(
        agents.map(async (agent) => ({ ...agent, cwd: await agentWorkingDirectory(agent.pid) })),
  ...
  if (url.pathname === '/api/agent-sessions/detail' && request.method === 'POST')
    return readJson(request, response, (body) =>
      json(
        response,
        agentSessions.detail({ id: String(body.id || ''), kind: String(body.kind || '') }),
      ),
    );
  if (url.pathname === '/api/skills' && request.method === 'GET')
    return json(response, { ok: true, skills });
  if (url.pathname === '/api/agents/open-project' && request.method === 'POST') {
    return handleAgentAction(request, response, (cwd) =>
  ...
  if (url.pathname === '/api/agents/resume' && request.method === 'POST') {
    return handleAgentAction(request, response, (cwd, kind) => {
      ...
      spawn('osascript', ['-e', resumeScript, cwd, command], {
        detached: true,
        stdio: 'ignore',
      }).unref();
    });
    ...
    try {
      ...
      if (message.type === 'start' && !session) {
        ...
        if (typeof message.cwd !== 'string')
          return ws.send(
            JSON.stringify({ type: 'error', message: 'Project directory unavailable' }),
          );
        const cwd = path.resolve(message.cwd);
        if (!cwd.startsWith(`${home}${path.sep}`) || !fs.existsSync(cwd))
          return ws.send(
            JSON.stringify({ type: 'error', message: 'Project directory unavailable' }),
          );
        const kind = message.kind === 'claude' ? 'claude' : 'codex';
        const sessionId =
          typeof message.sessionId === 'string' && /^[0-9a-f-]{16,80}$/i.test(message.sessionId)
            ? message.sessionId
            : '';
        const command =
          kind === 'claude'
            ? ['claude', '--resume', ...(sessionId ? [sessionId] : [])]
            : ['codex', 'resume', ...(sessionId ? [sessionId] : [])];
        session = pty.spawn(
          process.env.SHELL || '/bin/zsh',
          ['-lc', command.map((value) => `'${value.replace(/'/g, '\'\\"\'\\"')}'`).join(' ')],
          { name: 'xterm-256color', cols: 120, rows: 32, cwd, env: process.env },
        );
        session.onData(
          (data) =>
            ws.readyState === WebSocket.OPEN && ws.send(JSON.stringify({ type: 'data', data })),
        );
      ...
      if (message.type === 'input' && session && typeof message.data === 'string')
      ...
      if (message.type === 'resize' && session) {
        ...
        if (Number.isFinite(cols) && Number.isFinite(rows)) {
          session.resize(
            Math.min(1000, Math.max(20, Math.trunc(cols))),
            Math.min(1000, Math.max(5, Math.trunc(rows))),
          );
        }
      }
    } catch (_) {
...
function readJson(request, response, handler) {
  ...
  request.on('end', async () => {
    try {
      ...
      json(response, { ok: false, error: 'Invalid request.' });
    }
  });
}
...
function matchesFileIntent(file, query) {
  ...
  return (
    (!typeQueries.has(query.toLowerCase()) || name.endsWith(`.${query.toLowerCase()}`)) &&
...
function makeFileResult(file, query) {
  ...
  const folderRank =
    { Documents: 90, Desktop: 75, Downloads: 60, Projects: 35, Home: 15 }[folder] || 5;
  ...
  const starts =
    stem.startsWith(lowerQuery) || queryTokens.every((token) => stem.startsWith(token));
  const nameRank = exact
    ? 120
    : starts
      ? 80
      : queryTokens.reduce((score, token) => score + (stem.includes(token) ? 22 : 0), 0);
  const shallowRank = Math.max(
    0,
    20 - path.relative(process.env.HOME || '/', file).split(path.sep).length * 2,
  );
...
function relativeFileWindow(request) {
  ...
  const now = new Date();
  if (/\b(?:last night|yesterday(?: evening| night)?)\b/.test(text))
    return { after: new Date(now.getTime() - 36 * 60 * 60 * 1000), before: now };
  if (/\b(?:latest|recent|newest)\b/.test(text))
    return { after: new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000), before: now };
  return null;
}

function normalizedFileQuery(query) {
  return String(query || '')
    .replace(/\b(?:last|night|yesterday|today|latest|recent|newest|from|the|ones?|files?)\b/gi, ' ')
...
function findLocalFiles(query, limit = 8, { window = null } = {}) {
  ...
  if (window?.after instanceof Date)
    terms.push(`kMDItemFSContentChangeDate >= $time.iso(${window.after.toISOString()})`);
  if (window?.before instanceof Date)
    terms.push(`kMDItemFSContentChangeDate <= $time.iso(${window.before.toISOString()})`);
    ...
    finder.on('close', () => {
      ...
      resolve(
        [...fallback, ...spotlight]
          ...
          .sort(
            (left, right) => right.score - left.score || right.fileTimestamp - left.fileTimestamp,
          )
          .slice(0, limit),
      );
    });
...
function fileNameMatchesWindow(file, window) {
  ...
  const match = path
    .basename(file)
    .match(/(20\d{2}-\d{2}-\d{2})\s+(?:at\s+)?(\d{1,2}[.:]\d{2}(?:[.:]\d{2})?)/i);
  if (!match) return true;
  const candidate = new Date(`${match[1]}T${match[2].replace(/\./g, ':')}`);
  return (
    !Number.isNaN(candidate.getTime()) &&
    candidate >= window.after &&
    (!window.before || candidate <= window.before)
  );
}
...
function findLocalFilesFallback(query, limit, window) {
  const home = process.env.HOME || '';
  const directories = ['Desktop', 'Documents', 'Downloads'].map((folder) =>
    ...
    try {
      return fs
        ...
        .filter(
          (entry) =>
            entry.isFile() &&
            matchesFileIntent(entry.name, query) &&
            fileNameMatchesWindow(path.join(directory, entry.name), window),
        )
  ...
  return files
    .sort((left, right) => fileNameTimestamp(right) - fileNameTimestamp(left))
    .map((file) => ({ ...makeFileResult(file, query), fileTimestamp: fileNameTimestamp(file) }))
    .filter(Boolean)
    .sort((left, right) => right.score - left.score || right.fileTimestamp - left.fileTimestamp)
    .filter(uniqueFileName())
    .slice(0, limit);
}
...
function applicationMatchScore(name, query) {
  ...
  if (
    words.length &&
    words.every((word) => nameWords.some((candidate) => candidate.startsWith(word)))
  )
    return 820;
  ...
  const distance = Math.min(
    ...candidates.map((candidate) => boundedEditDistance(compactQuery, candidate, limit)),
  );
  return Number.isFinite(distance)
    ? 520 - distance * 85 - Math.max(0, compactName.length - compactQuery.length) * 3
    : 0;
}
  ...
  const refresh = () => {
    ...
    const discoverBundles = (root, depth = 0) => {
      ...
      return entries.flatMap((entry) => {
        ...
        const isBundle =
          entry.name.endsWith('.app') && (entry.isDirectory() || entry.isSymbolicLink());
    ...
    const apps = roots
      .flatMap((root) => discoverBundles(root))
      .filter((pathname) => {
      ...
      .filter(
        (app) =>
          !/(?:agent|assistant|daemon|service|helper|server|launcher|messenger|url handler|authwarn|accesscontrol|mac)$/i.test(
            app.name,
          ),
      );
    ...
    const visibleApps = apps.filter(
      (app) =>
        !app.path.startsWith('/System/Library/CoreServices/') ||
        ['Finder', 'Time Machine'].includes(app.name),
    );
    applicationIndex = { loadedAt: Date.now(), apps: visibleApps };
    return visibleApps;
  };
  const source =
    Date.now() - applicationIndex.loadedAt < 5 * 60_000 ? applicationIndex.apps : refresh();
    ...
    () =>
      new Promise((resolve) => {
        ...
        for (const app of applicationIndex.apps) {
          ...
          try {
            byPath.set(fs.realpathSync(app.path), app);
          } catch (_) {
            /* A removed app is ignored on refresh. */
          }
        }
        ...
        task.on('close', () => {
          ...
          for (const line of stdout.split('\n')) {
            ...
            const bundleMatch = command.match(
              /(\/(?:Applications|System\/Applications|System\/Cryptexes\/App\/System\/Applications|Users\/[^/]+\/Applications)\/.*?\.app)(?:\/|$)/,
            );
            ...
            if (!app) {
              try {
                app = byPath.get(fs.realpathSync(bundlePath));
              } catch (_) {
                /* Unknown bundle. */
              }
            }
            if (!app || app.name === 'Habibi') continue;
            const group = groups.get(app.path) || {
              name: app.name,
              path: app.path,
              pids: [],
              cpu: 0,
          ...
          resolve(
            [...groups.values()]
              .map((item) => ({
                ...
                memoryMb: Math.max(1, Math.round(item.memoryMb)),
              }))
              .sort(
                (a, b) => b.cpu - a.cpu || b.memoryMb - a.memoryMb || a.name.localeCompare(b.name),
              ),
          );
        });
      }),
...
function parseKubernetesReadQuery(input, preferredContext = '') {
  ...
  for (let index = 0; index < parts.length; index += 1) {
    ...
    if (part.startsWith('-'))
      throw new Error(`Flag ${part} is not available in Habibi's read-only Kubernetes plugin.`);
    values.push(part);
  }
  ...
  const resource =
    resourceAliases[(values[0] || '').toLowerCase()] || (values[0] || '').toLowerCase();
  ...
  const withScope = (args) => [
    ...args,
    ...(allNamespaces ? ['--all-namespaces'] : namespace ? ['--namespace', namespace] : []),
    ...(context ? ['--context', context] : []),
  ];
  if (verb === 'events' || (verb === 'get' && resource === 'events'))
    return { action: 'events', args: withScope(['get', 'events']), namespace, context };
  if (verb === 'logs') {
    const pod = values[0];
    if (!pod)
      throw new Error('Say “logs <pod> -n <namespace>”, for example: logs api-7c9d -n production.');
    return {
      action: 'logs',
      args: [
        ...withScope(['logs', pod]),
  ...
  if (!allowedResources.has(resource))
    throw new Error(
      'Use a read-only query such as “get pods”, “describe deployment api”, “logs api-pod -n production”, or “events -n production”.',
    );
  if (verb === 'get')
    return {
      action: 'get',
      args: withScope(['get', resource, ...(values[1] ? [values[1]] : []), '--output', 'wide']),
      namespace,
      context,
    };
  if (verb === 'describe') {
    if (!values[1])
      throw new Error(
        'Describe needs a resource and name, for example: describe deployment api -n production.',
      );
    return {
      action: 'describe',
      args: withScope(['describe', resource, values[1]]),
      namespace,
      context,
    };
  }
  throw new Error(
    'Habibi only runs read-only Kubernetes commands: get, describe, logs, and events.',
  );
}
...
function appendKubernetesAudit({ action, namespace, context, exitCode }) {
  try {
    ...
    fs.appendFileSync(
      ...
      `${JSON.stringify({ at: new Date().toISOString(), action, namespace: namespace || null, context: context || null, exit: exitCode })}\n`,
      { mode: 0o600 },
    );
  } catch (_) {
    /* Auditing must never make a read-only inspection unavailable. */
  }
}
...
function runKubectlRead(args, audit, { maxOutputBytes = 500_000 } = {}) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      task.kill('SIGTERM');
      finish({ ok: false, error: 'kubectl timed out after 10 seconds.' }, 124);
    }, 10_000);
    task.stdout.on('data', (chunk) => {
      stdout += chunk;
      ...
      finish(
        {
          ...
          error:
            error.code === 'ENOENT'
              ? 'kubectl is not installed or not on Habibi’s PATH.'
              : 'Could not start kubectl.',
        },
        127,
      );
    });
      ...
      finish(
        outputTooLarge
          ? {
              ok: false,
              error:
                'This Kubernetes response is too large to display safely. Narrow the namespace or resource.',
            }
          : code === 0
            ? { ok: true, output: stdout }
            : { ok: false, error: (stderr || 'kubectl failed.').trim().slice(0, 4_000) },
        code ?? 1,
      );
    });
  });
...
function runKubernetesReadQuery(input, preferredContext = '', preferredNamespace = '') {
  let parsed;
  try {
    parsed = parseKubernetesReadQuery(input, preferredContext);
  } catch (error) {
  ...
  if (preferredNamespace && !parsed.namespace && !parsed.args.includes('--all-namespaces')) {
    ...
    parsed.args.splice(
      insertion < 0 ? parsed.args.length : insertion,
      0,
      '--namespace',
      preferredNamespace,
    );
  ...
  return runKubectlRead(parsed.args, parsed).then((result) => ({
    ...result,
...
async function kubernetesOverview(requestedContext, requestedNamespace = '') {
  const contextsResult = await runKubectlRead(['config', 'get-contexts', '-o', 'name'], {
    action: 'contexts',
    namespace: '',
    context: '',
  });
  if (!contextsResult.ok)
    return { ok: false, error: contextsResult.error, contexts: [], resources: [] };
  const contexts = contextsResult.output
    .split('\n')
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, 100);
  const currentResult = await runKubectlRead(['config', 'current-context'], {
    action: 'current-context',
    namespace: '',
    context: '',
  });
  const current = currentResult.ok ? currentResult.output.trim() : '';
  const context = contexts.includes(requestedContext)
    ? requestedContext
    : contexts.includes(current)
      ? current
      : contexts[0] || '';
  if (requestedNamespace && !validKubernetesName(requestedNamespace, 62))
    return { ok: false, error: 'That namespace is not valid.', contexts, resources: [] };
  const scoped = (args) => [...args, ...(context ? ['--context', context] : [])];
  const namespacesResult = await runKubectlRead(
    scoped(['get', 'namespaces', '--output', 'json']),
    { action: 'namespaces', namespace: '', context },
    { maxOutputBytes: 1_000_000 },
  );
  const namespaces = namespacesResult.ok
    ? (() => {
        try {
          return JSON.parse(namespacesResult.output)
  ...
  const specs = [
    ['pods', ['get', 'pods', ...scope, '--output', 'json']],
    ['deployments', ['get', 'deployments', ...scope, '--output', 'json']],
    ['services', ['get', 'services', ...scope, '--output', 'json']],
  ];
  const resources = await Promise.all(
    specs.map(async ([kind, args]) => {
      ...
      const result = await runKubectlRead(
        scoped(args),
        { action: `overview-${kind}`, namespace, context },
      ...
      try {
        return {
          kind,
          ok: true,
          items: kubernetesOverviewItems(kind, JSON.parse(result.output).items || []),
        };
      } catch (_) {
        return { kind, ok: false, error: `Could not read ${kind} returned by kubectl.`, items: [] };
      }
    }),
  );
...
function kubernetesOverviewItems(kind, items) {
  return items.slice(0, 80).map((item) => {
    ...
    if (kind === 'pods') {
      ...
      return {
        ...base,
        primary: status.phase || 'Unknown',
        secondary: `${ready}/${containers.length || 0} ready`,
        badge: containers.reduce((total, container) => total + (container.restartCount || 0), 0)
          ? `${containers.reduce((total, container) => total + (container.restartCount || 0), 0)} restarts`
          : '',
      };
    }
    if (kind === 'deployments')
      return {
        ...base,
        primary: `${status.readyReplicas || 0}/${spec.replicas || 0} ready`,
        secondary: status.availableReplicas
    ...
    if (kind === 'services')
      return {
        ...base,
        primary: spec.type || 'ClusterIP',
        secondary:
          (spec.ports || [])
            .map((port) => `${port.port}${port.name ? `/${port.name}` : ''}`)
            .join(' · ') || 'No ports',
        badge: spec.clusterIP && spec.clusterIP !== 'None' ? spec.clusterIP : '',
      };
    ...
    return {
      ...base,
      primary: ready ? `${ready.type}: ${ready.status}` : status.phase || item.kind || 'Resource',
      secondary: metadata.creationTimestamp
...
function validKubernetesName(value, max = 252) {
  return (
    /^[A-Za-z0-9][A-Za-z0-9._-]{0,252}$/.test(String(value || '')) && String(value).length <= max
  );
}
function kubernetesScope(kind, namespace, context, allNamespaces = false) {
  return [
    ...(allNamespaces && !['nodes', 'namespaces'].includes(kind)
      ? ['--all-namespaces']
      : namespace && !['nodes', 'namespaces'].includes(kind)
        ? ['--namespace', namespace]
        : []),
    ...(context ? ['--context', context] : []),
  ];
...
async function kubernetesResourceList(payload = {}) {
  ...
  if (!kind)
    return {
      ok: false,
      error: 'That Kubernetes resource is not available in the read-only explorer.',
    };
  if (namespace && !validKubernetesName(namespace, 62))
    return { ok: false, error: 'That namespace is not valid.' };
  const result = await runKubectlRead(
    ['get', kind, ...kubernetesScope(kind, namespace, context, !namespace), '--output', 'json'],
    { action: `browse-${kind}`, namespace, context },
    { maxOutputBytes: 4_000_000 },
  );
  ...
  try {
    return {
      ok: true,
      kind,
      items: kubernetesOverviewItems(kind, JSON.parse(result.output).items || []).slice(0, 150),
    };
  } catch (_) {
    return { ok: false, error: `Could not read ${kind} returned by kubectl.` };
  }
}
...
async function kubernetesResourceDetail(payload = {}) {
  ...
  const namespace = String(payload.namespace || '').trim();
  const context = String(payload.context || '').trim();
  if (!kind || !validKubernetesName(name))
    return { ok: false, error: 'That resource is not available for inspection.' };
  if (namespace && !validKubernetesName(namespace, 62))
    return { ok: false, error: 'That namespace is not valid.' };
  const args = [
    'get',
    kind,
    name,
    ...kubernetesScope(kind, namespace, context),
    '--output',
    'json',
  ];
  ...
  try {
    ...
    const selector = Object.entries(value.spec?.selector?.matchLabels || {})
      .map(([key, label]) => `${key}=${label}`)
      .join(',');
    if (
      selector &&
      ['deployments', 'jobs', 'statefulsets', 'daemonsets', 'replicasets'].includes(kind) &&
      namespace
    ) {
      const pods = await runKubectlRead(
        [
          'get',
          'pods',
          '--namespace',
        ...
        { action: `related-pods-${kind}`, namespace, context },
        { maxOutputBytes: 1_500_000 },
      );
      if (pods.ok)
        detail.relatedPods = kubernetesOverviewItems(
          'pods',
          JSON.parse(pods.output).items || [],
        ).slice(0, 50);
    }
    return { ok: true, detail };
  } catch (_) {
    return { ok: false, error: 'kubectl returned a resource Habibi could not read.' };
  }
}
...
async function kubernetesPodLogs(payload = {}) {
  ...
  if (
    !validKubernetesName(pod) ||
    !validKubernetesName(namespace, 62) ||
    (container && !validKubernetesName(container))
  )
    return { ok: false, error: 'Choose a valid pod and namespace to read logs.' };
  const result = await runKubectlRead(
    [
      'logs',
      pod,
...
function formatKubernetesDetail(kind, value) {
  ...
  const containers = (spec.containers || []).slice(0, 30).map((container) => {
    ...
    return {
      ...
      state: Object.keys(state)[0] || (runtime.ready ? 'Running' : 'Unknown'),
      ready: Boolean(runtime.ready),
      restarts: runtime.restartCount || 0,
    };
  });
  const conditions = (status.conditions || []).slice(0, 12).map((condition) => ({
  ...
  if (kind === 'deployments')
    facts.push(
      ...
      ['Updated', String(status.updatedReplicas || 0)],
    );
  if (kind === 'services')
    facts.push(
      ...
      [
        'Ports',
        (spec.ports || []).map((port) => `${port.port}/${port.protocol || 'TCP'}`).join(' · ') ||
          '—',
      ],
    );
  if (!facts.length)
    facts.push(
      ...
      ['Labels', String(Object.keys(metadata.labels || {}).length)],
      ['Annotations', String(Object.keys(metadata.annotations || {}).length)],
    );
  return {
    kind,
    name: metadata.name || 'Unnamed resource',
...
async function planKubernetesNaturalQuery(query, preferredContext, preferredNamespace = '') {
  ...
  return {
    ...
    trace: [
      {
        tool: 'Local intent planner',
        detail: 'Translated your request into a constrained read-only query.',
      },
      {
        tool: 'kubectl read',
        detail: result.ok
          ? 'Read completed using your selected context.'
          : 'The read was rejected or could not complete.',
      },
    ],
  };
}
...
async function diagnoseKubernetes(query, preferredContext, preferredNamespace = '') {
  try {
    ...
    return {
      ok: false,
      error:
        'The Kubernetes investigation could not complete. Check the selected context and try again.',
    };
  }
}
