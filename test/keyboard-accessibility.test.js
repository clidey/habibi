...
test('command arrows fall back to the shared keyboard loop on every non-result surface', () => {
  const controller = fs.readFileSync(path.join(process.cwd(), 'src/client/core/keyboard-controller.js'), 'utf8');
...
test('entering a result list from the command input starts at the first row', () => {
  const controller = fs.readFileSync(path.join(process.cwd(), 'src/client/core/keyboard-controller.js'), 'utf8');
  assert.match(controller, /focused \? items\.indexOf\(focused\) : \(enterFromInput \? -1 : selectedIndex\)/);
});
