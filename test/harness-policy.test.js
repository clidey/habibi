...
test('harness allows reads but stops all unapproved writes', () => {
  ...
  assert.equal(evaluateToolCall({ name:'calendar_create_event', redeemApproval:() => true }).decision, 'allow');
  ...
  assert.equal(evaluateToolCall({ name:'calendar_create_event', redeemApproval:() => false }).decision, 'require_confirmation');
});
