...
test('launching an imported skill passes hostile input as data, never as AppleScript source', async () => {
  ...
  const service = createSkillImportService({
    root, stateRoot:root, home:path.join(root, 'home'),
    spawn:(program, args) => { spawns.push({ program, args }); return { unref() {} }; },
  });
  ...
  const promptFile = args.find(value => typeof value === 'string' && value.endsWith('prompt.txt'));
  assert.ok(promptFile, 'the prompt should be passed as a file path');
  assert.equal(args.some(value => String(value).includes('do shell script')), false);
