...
function successfulKeychain() {
  return () => ({
    stdout:{ on:(event, handler) => { if (event === 'data') handler('saved-key'); } },
    stderr:{ on:() => {} },
    stdin:{ end:() => {} },
...
function withService(run) {
  ...
  try { return run(createLlmService({ root, fs, spawn:successfulKeychain() })); }
...
test('OpenAI setup verifies a supplied key before saving it', async () => {
  await withService(async service => {
    ...
    try {
      const result = await service.configure({ provider:'openai', model:' chat-latest ', apiKey:' sk-test ' });
