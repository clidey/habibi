...
test('a session stuck in "created" past the threshold is force-killed, deleted, and replaced', async () => {
  await withFakeOpenwa(
    [{ id: '1', name: 'habibi', status: 'created', engineLoaded: false }],
    async ({ baseUrl, workspace, calls, getSessions }) => {
      ...
      assert.equal(
        first.session.id,
        '1',
        'an unstale session must be returned as-is, not healed on first sight',
      );
      assert.equal(
        calls.some((call) => call.includes('force-kill')),
        false,
      );
      ...
      assert.notEqual(
        second.session.id,
        '1',
        'the stuck session must be replaced, not returned again',
      );
      assert.equal(
        second.session.status,
        'created',
        'the replacement is a fresh session, not the healed one resurrected',
      );
      assert.ok(
        calls.some((call) => call === 'POST /api/sessions/1/force-kill'),
        'must force-kill the stuck session',
      );
      assert.ok(
        calls.some((call) => call === 'DELETE /api/sessions/1'),
        'must delete the stuck session',
      );
      assert.equal(
        getSessions().some((session) => session.id === '1'),
        false,
        'the stuck session must actually be gone',
      );
    },
  );
});
  ...
  await withFakeOpenwa(
    ...
    async ({ baseUrl, workspace, calls }) => {
      ...
      assert.equal(
        calls.some((call) => call.includes('force-kill')),
        false,
        'a disconnected session must never be force-killed',
      );
    },
  );
});
...
test('a session already making progress resets the staleness timer', async () => {
  await withFakeOpenwa(
    ...
    async ({ baseUrl, workspace, calls, getSessions }) => {
      ...
      assert.equal(
        afterProgress.session.id,
        '1',
        'a session that reached qr_ready must never be healed',
      );
      ...
      assert.equal(
        justRegressed.session.id,
        '1',
        'immediately after regressing, healing must not fire yet',
      );
      assert.equal(
        calls.some((call) => call.includes('force-kill')),
        false,
...
test('ensureSession heals a stuck session the same way sessionState does', async () => {
  await withFakeOpenwa(
    [{ id: '1', name: 'habibi', status: 'failed', engineLoaded: true }],
    async ({ baseUrl, workspace, calls, getSessions }) => {
      const client = createOpenwaClient({ workspace, baseUrl, staleAfterMs: 10 });
      ...
      assert.equal(
        getSessions().some((session) => session.name === 'habibi'),
        true,
        'a fresh session must exist after healing',
      );
    },
  );
});
...
test('a real, previously-linked disconnected session gets its engine restarted, never destroyed', async () => {
  ...
  await withFakeOpenwa([linked], async ({ baseUrl, workspace, calls, getSessions }) => {
    ...
    assert.equal(
      state.session.phone,
      '447426315115',
      'the real linked device info must be preserved',
    );
    assert.equal(
      calls.some((call) => call.includes('force-kill')),
      false,
    ...
    assert.ok(
      calls.some((call) => call === 'POST /api/sessions/1/start'),
      'the engine must be restarted for a disconnected, not-yet-loaded session',
    );
    assert.equal(
      getSessions()[0].engineLoaded,
      true,
      'the same session row must now show its engine loaded',
    );
  });
});
...
test("restarting a disconnected session's engine is throttled, not retried on every poll", async () => {
  ...
  await withFakeOpenwa([linked], async ({ baseUrl, workspace, calls, getSessions }) => {
    ...
    assert.equal(
      startCalls.length,
      1,
      'a second poll inside the throttle window must not restart the engine again',
    );
  });
});
...
test('forceReset force-kills and replaces a session immediately, bypassing the staleness timer', async () => {
  ...
  await withFakeOpenwa([linked], async ({ baseUrl, workspace, calls, getSessions }) => {
    ...
    assert.ok(
      calls.some((call) => call === 'POST /api/sessions/1/force-kill'),
      'must force-kill the existing session immediately',
    );
    assert.ok(
      calls.some((call) => call === 'DELETE /api/sessions/1'),
      'must delete the existing session',
    );
    assert.equal(
      getSessions().some((session) => session.id === '1'),
      false,
      'the old session row must be gone',
    );
    assert.equal(
      getSessions().some((session) => session.name === 'habibi'),
      true,
      'a fresh session must exist after resetting',
    );
  });
});
  ...
  await withFakeOpenwa(
    ...
    async ({ baseUrl, workspace, calls }) => {
      ...
      assert.equal(
        killCalls.length,
        1,
        'forceReset itself must be the only thing that force-kills session 1',
      );
      ...
      assert.equal(
        calls.filter((call) => call.includes('force-kill')).length,
        1,
        'the freshly created session must not be force-killed again right away',
      );
    },
  );
});
