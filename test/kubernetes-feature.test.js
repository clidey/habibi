...
test('Kubernetes controller owns its view, routing callbacks, and queries', async () => {
  const dom = new JSDOM('<input id="command"><main id="home"></main><main id="results" class="hidden"></main><span id="count"></span>', { url:'http://127.0.0.1:4173/' });
  ...
  globalThis.fetch = async (url, options = {}) => {
    ...
    if (String(url).startsWith('/api/kubernetes/overview')) return { json:async () => ({ ok:true, context:'dev', contexts:['dev'], namespace:'', namespaces:['default'], resources:[] }) };
    return { json:async () => ({ ok:true, action:'get', output:'pod/api Ready' }) };
  };
  let opened = 0;
  let backed = 0;
  const { createKubernetesFeature } = await import('../src/client/features/kubernetes/kubernetes-feature.js');
