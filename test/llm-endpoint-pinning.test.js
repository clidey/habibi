...
const withService = run => {
  ...
  const spawn = () => {
    const child = {
      stdout:{ on:(event, handler) => { if (event === 'data') handler('stored-api-key'); } },
      stderr:{ on:() => {} },
      stdin:{ end:() => {} },
  ...
  try { return run({ service:createLlmService({ root, fs, spawn }), root }); }
...
test('a hosted provider endpoint cannot be redirected by the caller', async () => {
  await withService(async ({ service, root }) => {
    ...
    // pick it up.
    const stored = JSON.parse(fs.readFileSync(path.join(root, '.habibi', 'llm-config.json'), 'utf8'));
...
test('a config file poisoned before the guard existed is re-pinned on read', async () => {
  await withService(async ({ service, root }) => {
    ...
    fs.writeFileSync(path.join(root, '.habibi', 'llm-config.json'), JSON.stringify({
      provider:'anthropic', model:'claude-sonnet-4-5', endpoint:'https://attacker.example',
    }));
...
test('a local provider may move, but only to loopback', async () => {
  await withService(async ({ service }) => {
    const moved = await service.configure({ provider:'ollama', endpoint:'http://127.0.0.1:11500' });
    assert.equal(moved.endpoint, 'http://127.0.0.1:11500');

    const named = await service.configure({ provider:'ollama', endpoint:'http://localhost:11434' });
    assert.equal(named.endpoint, 'http://localhost:11434');

    // Anything that leaves the machine falls back to the pinned default.
    for (const endpoint of ['http://evil.tld:11434', 'https://attacker.example', 'http://169.254.169.254', 'file:///etc/passwd', 'not-a-url']) {
      const rejected = await service.configure({ provider:'ollama', endpoint });
      assert.equal(rejected.endpoint, PROVIDERS.ollama.endpoint, `${endpoint} should not be accepted`);
    }
  });
});
