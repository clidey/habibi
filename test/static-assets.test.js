...
test('the static handler serves only the launcher assets', () => {
  for (const asset of ['/', '/index.html', '/app.css', '/assets/app.bundle.js', '/assets/logo.png']) {
...
test('local secrets and source files are never reachable over HTTP', () => {
  ...
  for (const target of blocked) assert.equal(resolveStaticAsset(target, root), null, `${target} must not be served`);
});

test('traversal attempts cannot escape the workspace root', () => {
  for (const target of ['/../secrets.json', '/../../etc/passwd', '/%2e%2e/secrets.json', '//../secrets.json', '/assets/../../.env']) {
  ...
  assert.equal(resolveStaticAsset('/index.html', '/workspace/habibi'), '/workspace/habibi/index.html');
