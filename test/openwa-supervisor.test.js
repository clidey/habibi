...
test('the OpenWA supervisor kills its service after the Habibi parent exits', async () => {
  ...
  fs.writeFileSync(
    service,
    `require('node:fs').writeFileSync(process.env.OPENWA_TEST_PID_FILE, String(process.pid)); setInterval(() => {}, 1000);`,
  );
  ...
  try {
    ...
    assert.equal(
      isAlive(servicePid),
      false,
      'the service must exit when its native parent disappears',
    );
  } finally {
