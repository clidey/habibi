...
const withService = async (accountSecret, run) => {
  ...
  const spawn = (program, args) => ({
    stdout: {
      on: (event, handler) => {
        if (event === 'data' && accountSecret) handler(accountSecret);
      },
    },
    stderr: { on: () => {} },
    stdin: { end: () => {} },
    on: (event, handler) => {
      if (event === 'close') handler(accountSecret ? 0 : 1);
    },
  });
...
const withAccount = (service, root, provider, email) => {
  ...
  fs.writeFileSync(
    path.join(root, '.habibi', 'mail-providers.json'),
    JSON.stringify({
      accounts: {
        [id]: {
          provider,
          imap: { email, host: 'imap.example.test', port: 993 },
...
test('sending requires a connected account, a valid recipient, and a body', async () => {
  await withService(null, async (service, root) => {
    ...
    const noAccount = await service.send({
      provider: 'unknown-account',
      to: 'a@example.test',
      subject: 'hi',
      body: 'hello',
    });
    ...
    const noSecret = await service.send({
      provider: id,
      to: 'a@example.test',
      subject: 'hi',
      body: 'hello',
...
test('an invalid recipient or an empty body is rejected before any network call', async () => {
  await withService('app-password', async (service, root) => {
    ...
    const badAddress = await service.send({
      provider: id,
      to: 'not-an-email',
      subject: 'hi',
      body: 'hello',
    ...
    const emptyBody = await service.send({
      provider: id,
      to: 'a@example.test',
      subject: 'hi',
      body: '   ',
