...
test('analytics forwards only allowlisted anonymous product fields', async () => {
  ...
  const captured = await analytics.capture({
    ...
    properties: {
      ...
      query: 'passport for Alice',
      filename: '/Users/alice/visa.pdf',
  ...
  assert.deepEqual(body.properties, {
    distinct_id: '5beee387-4b03-49c6-8ca0-96bd64372088',
    product: 'habibi',
    query_length_bucket: '21-80',
    has_attachments: false,
  });
...
test('analytics rejects unknown events and malformed identities', async () => {
  ...
  assert.equal(
    await analytics.capture({ event: 'habibi.chat.prompt', distinctId: 'not-an-id' }),
    false,
  );
});
