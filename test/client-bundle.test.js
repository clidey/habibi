...
test('native launcher corners use the intended treatment for every theme', () => {
  ...
  assert.ok(
    Math.max(
      stylesheet.lastIndexOf("body.native-host:not([data-theme='deep-ocean']) .command-card"),
      stylesheet.lastIndexOf('body.native-host:not([data-theme="deep-ocean"]) .command-card'),
    ) >
      Math.max(
        stylesheet.lastIndexOf("body[data-theme='boring-good'] .command-card"),
        stylesheet.lastIndexOf('body[data-theme="boring-good"] .command-card'),
      ),
  ...
  for (const theme of themes) {
    const dom = new JSDOM(
      `<style>${stylesheet}</style><body class="native-host" data-theme="${theme}"></body>`,
    ...
    assert.equal(
      ...
      'rgba(0, 0, 0, 0)',
      `${theme} must not paint a rectangular native background`,
    );
    assert.equal(
      style.backgroundImage,
      'none',
      `${theme} must not leave a page gradient behind the launcher`,
    );
    dom.window.close();
  }
});
...
test('the built client bundle boots without runtime errors', async () => {
  ...
  dom.window.fetch = async () => ({
    ok: true,
    json: async () => ({ ok: false, skills: [], threads: [], events: [], agents: [], apps: [] }),
    text: async () => '',
  });
...
test('the sanitizing render path strips script while keeping launcher markup', async () => {
  ...
  try {
    ...
    setHtml(
      host,
      '<button class="result" data-type="chat" data-title="Amina"><span class="result-title">Amina</span></button>',
    );
    ...
    assert.equal(
      host.querySelector('#analytics-enabled')?.checked,
      true,
      'sanitizing settings markup must preserve checked state',
    );

    setHtml(host, '<input type="file" id="whatsapp-file-input" multiple hidden>');
    assert.equal(
      host.querySelector('#whatsapp-file-input')?.hidden,
      true,
      'native file controls must remain hidden behind the styled attachment button',
    );
    assert.equal(
      host.querySelector('#whatsapp-file-input')?.multiple,
      true,
      'attachment pickers must retain multi-file selection',
    );
  } finally {
...
test('reopening Home reuses the recent proactive context', async () => {
  ...
  dom.window.fetch = async (url) => {
    if (String(url).startsWith('/api/mail/status'))
      return { json: async () => ({ ok: true, accounts: [{ connected: true }] }) };
    if (String(url).startsWith('/api/mail/recent')) {
      recentRequests += 1;
      return { json: async () => ({ ok: true, threads: [] }) };
    }
