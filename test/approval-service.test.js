...
test('an approval is bound to one action and one exact payload', () => {
  ...
  assert.equal(
    approvals.consume({
      ...
      payload: { chatId: '42@c.us', text: 'send me your password' },
    }),
    false,
  );
  ...
  assert.equal(
    approvals.consume({ token: reissued.token, action: 'whatsapp.send', payload: { ...message } }),
    true,
  );
});
...
test('approvals are single use, action-bound, key-order independent, and expire', () => {
  ...
  assert.equal(
    approvals.consume({ token: spent.token, action: 'calendar.create', payload: event }),
    true,
  );
  assert.equal(
    approvals.consume({ token: spent.token, action: 'calendar.create', payload: event }),
  ...
  assert.equal(
    approvals.consume({ token: probed.token, action: 'calendar.update', payload: event }),
    false,
  );
  assert.equal(
    approvals.consume({ token: probed.token, action: 'calendar.create', payload: event }),
    false,
  );
  ...
  assert.equal(
    approvals.consume({
      token: reordered.token,
      action: 'calendar.create',
      payload: { end: event.end, start: event.start, calendar: event.calendar, title: event.title },
    }),
    true,
  );
  ...
  assert.equal(
    approvals.consume({ token: 'not-a-real-token', action: 'calendar.create', payload: event }),
    false,
  );

  const expired = createApprovalService({ ttlMs: -1 }).issue('system.restart', {
    action: 'restart',
  });
  assert.equal(
    createApprovalService({ ttlMs: -1 }).consume({
      token: expired.token,
