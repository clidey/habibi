...
const service = createWhatsAppService({
  ...
  openwaClient:{ request:async () => ({}), sessionState:async () => ({ session:null }), ensureSession:async () => {} },
});
...
test('collapses duplicate connector rows by chat id and keeps the newest preview', () => {
  const [chat] = service.dedupeChats([
    ...
    { id:'447700@c.us', name:'Alex', timestamp:20, lastMessage:'newer', unreadCount:0, avatar:'https://example.test/avatar.jpg' },
  ]);
...
test('accepts only a timestamped visible recents snapshot', () => {
  const valid = Array.from({ length:5 }, (_, index) => ({ id:`${index}@c.us`, timestamp:index + 1, kind:'individual' }));
